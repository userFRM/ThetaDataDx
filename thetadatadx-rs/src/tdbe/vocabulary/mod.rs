...
/// vocabulary can resolve the three that were renamed here.
///
/// ```
...
mod tests {
    ...
    #[test]
    fn the_vocabularies_are_populated() {
        assert_eq!(DATA_TYPE.len(), 91);
        assert_eq!(REQ_TYPE.len(), 40);
        assert_eq!(REQ_ARG.len(), 29);
        assert_eq!(RESULTS_FORMAT.len(), 6);
        assert_eq!(ACCOUNT_TYPE.len(), 4);
        assert_eq!(CALENDAR_TYPE.len(), 2);
    }
}
