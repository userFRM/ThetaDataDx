//! Emit `thetadatadx-py/src/_generated/tick_classes.rs` — typed `#[pyclass]` structs,
...
/// Renders `thetadatadx-py/src/_generated/tick_classes.rs` — the typed `#[pyclass]` struct, `<Tick>List` wrapper, iterator, and list converters per tick type, plus the shared `StringList` wrapper and class registration helper.
...
fn render_python_string_list() -> String {
    ...
    out.push_str("///\n");
    out.push_str("/// `unsendable` opts the iterator into PyO3's single-thread affinity\n");
    out.push_str("/// check under free-threaded Python (PEP 703). The cursor is a plain\n");
    out.push_str("/// `usize` advanced by `&mut self` inside `__next__`; without the\n");
    out.push_str("/// marker a nogil interpreter would let two threads drive the same\n");
    out.push_str("/// iterator concurrently and lose ticks. Pythonic iterators are\n");
    out.push_str("/// single-thread by contract — the affinity check enforces it at\n");
    out.push_str("/// the binding boundary.\n");
    out.push_str("#[pyclass(module = \"thetadatadx\", unsendable, skip_from_py_object)]\n");
...
fn render_python_tick_class_struct(type_name: &str, def: &TickTypeDef) -> String {
    ...
    // hand-written Rust `impl` in `thetadatadx-rs/src/tdbe/types/tick.rs`.
