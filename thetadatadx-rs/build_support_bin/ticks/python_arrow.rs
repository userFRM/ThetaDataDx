...
//! exported as an `FFI_ArrowArrayStream` and imported through the PyCapsule
//! protocol, `RecordBatchReader._import_from_c_capsule`, with an address-based
//! `_import_from_c` fallback for pyarrow < 14). pandas 2.x consumes those
//! buffers in place for all numeric dtypes.
...
/// and public helper (`<tick>_slice_to_arrow_table`) have a live caller. Every
/// `<Tick>List` terminal projects through the `_projected` pair, so the only
/// consumer of a full reader is the streaming bench's `TradeTick` fast path.
/// Emission is gated on this set so the other ~38 readers are simply not
/// generated rather than emitted behind a blanket `#[allow(dead_code)]`.
const FULL_READER_CONSUMERS: &[&str] = &["TradeTick"];
...
pub(super) fn render_python_tick_arrow(schema: &Schema) -> String {
    ...
    out.push_str("// pyarrow via the Arrow C Stream Interface (`FFI_ArrowArrayStream`), handed\n");
...
fn render_python_slice_to_arrow_converters(schema: &Schema) -> String {
    ...
    for type_name in sorted_type_names(schema) {
        ...
        // Full reader + public helper: emit ONLY for tick types a caller
        // actually reaches through the non-projected path. Every `<Tick>List`
        // terminal projects (uses the `_projected` pair below), so the only
        // live consumer of a full reader is the streaming bench's
        // `trade_tick_slice_to_arrow_table` fast path. Gating emission on the
        // real consumer set drops ~38 otherwise-dead generated functions
        // instead of blanketing them with `#[allow(dead_code)]`.
        if FULL_READER_CONSUMERS.contains(&type_name) {
...
fn render_python_slice_public_helper(type_name: &str) -> String {
    ...
    .unwrap();
    // The column build is pure Rust and O(n); release the GIL across it so a
    // sibling Python thread keeps running while a large slice materialises.
    writeln!(out, "    let batch = py.detach(|| {reader}(ticks))?;").unwrap();
...
fn render_python_slice_public_helper_projected(type_name: &str) -> String {
    ...
    .unwrap();
    // The projected column build is pure Rust and O(n); release the GIL across
    // it so a sibling Python thread keeps running while a large slice builds.
    writeln!(
        out,
        "    let batch = py.detach(|| {reader}(ticks, present))?;"
    )
    .unwrap();
    out.push_str("    record_batch_to_pyarrow_table(py, batch)\n");
    out.push_str("}\n");
