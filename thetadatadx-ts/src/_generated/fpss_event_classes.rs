...
// Typed #[napi(object)] structs for every `StreamData` and `StreamControl`
// variant + a flat `StreamEvent` wrapper and the `buffered_event_to_typed`
...
// `thetadatadx-ts/src/lib.rs`), so importing it here would collide.
...
pub(crate) fn buffered_event_to_typed(event: BufferedEvent) -> StreamEvent {
    ...
    match event {
        BufferedEvent::MarketValue {
            ...
            out.kind = "market_value";
            out.market_value = Some(MarketValue {
                contract: Contract {
