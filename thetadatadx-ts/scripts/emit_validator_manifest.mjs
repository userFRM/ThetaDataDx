...
const DTS_PATH = path.join(REPO_ROOT, 'thetadatadx-ts', 'index.d.ts');
...
function parseMethods(source) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^export declare class (Client|MarketDataView|StreamView)\b/.test(trimmed)) {
      inClass = true;
      depth = 1;
      continue;
    }
...
function buildRecords(interfaces, methods) {
  ...
  for (const [methodName, cell] of Object.entries(METHOD_TO_CELL)) {
    ...
    records.push({
      ...
      row_count: 1,
      duration_ms: 0,
      detail: 'shape manifest extracted from thetadatadx-ts/index.d.ts',
      first_row: firstRow,
    });
  }
