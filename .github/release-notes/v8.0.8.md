...
- A broken intra-doc link on `FitReader`'s module-level docstring now resolves.
- Five redundant explicit link targets on `[Error::Config]` references in the `tdbe` right-parser docs dropped; rustdoc resolves the bare path.
...
- Two doc references to the old `tdbe` error module repointed to `tdbe::error`.
- A stale normalization-module path in the parity checklist updated to its current location after the v8.0.7 fold.
- A stale normalization-module parenthetical removed from an internal module docstring.
...
- `docs-site/docs/.vitepress/config.ts` — `vite.build.chunkSizeWarningLimit` raised to `1500` kB. The docs site bundles Mermaid and Vue chunks that exceed the default 500 kB threshold; the warning was non-actionable.
- `deny.toml` — unused license allowances pruned from `[licenses].allow`; remaining entries carry a short comment explaining why each is there. A few upstream-rooted duplicate-version dependencies are listed under `[bans].skip-tree`. `cargo deny check` now produces zero warnings.
