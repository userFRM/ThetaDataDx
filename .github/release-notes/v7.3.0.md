...
- **TypeScript/Node.js SDK via napi-rs** (#332) -- native Node.js addon exposing all 61 historical endpoints, the streaming method surface, and 13 tick types, SSOT-generated from the same TOML surface that drives the Python, Go, and C++ SDKs. TypeScript type definitions included. CI builds and smoke-tests on every PR. TypeScript SDK available from source; npm publish workflow coming in a follow-up.

## Fixed

- **FPSS auto-reconnect now re-subscribes all active contracts** (#333) -- the reconnect path authenticated but never re-sent subscription frames after an involuntary disconnect, causing data to stop flowing. All active subscriptions are now re-sent after reconnect login.
...
- Subscription state is now shared with the reconnect path so it can re-subscribe without a command-channel round-trip (#333).
