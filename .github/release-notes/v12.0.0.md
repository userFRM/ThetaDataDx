...
Major release. The FPSS streaming surface is reshaped around a fluent builder, a typed `FpssError` enum, and an iterator on the client itself. The three earlier delivery modes (push callback, pull iterator, and a caller-driven poller) collapse to one primitive. The companion `tdbe` crate is unchanged at `0.14.0`.
...
- One runtime queue dependency removed.
...
- The streaming I/O internals are simpler, with the prior queue-handoff layer removed; behavior is unchanged.
