...
fn count_go_tls_reader_methods(
    ...
    // The self-pinning helpers ARE the TLS reads — they are what the markers
    // point at. Their callers are the ones counted; skip the helpers
    // themselves. Kept in lockstep with `isTLSHelper` in timeout_pin_test.go.
    const TLS_HELPERS: &[&str] = &["lastError", "lastErrorRaw", "fpssCall"];
    ...
    for (file_name, path) in files {
        ...
        for (idx, line) in lines.iter().enumerate() {
            ...
            if TLS_HELPERS.contains(&method_name) {
                continue;
            }
