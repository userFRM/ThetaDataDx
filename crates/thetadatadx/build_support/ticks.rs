...
/// columnar converter — everything else routes through the generic
/// `pyclass_list_to_columnar` pivot on the typed pyclass list. Keeping
/// this list tight avoids generating dead code that would otherwise
/// require `#[allow(dead_code)]` to ship under `-D warnings`.
const PYTHON_TICK_COLUMNAR_TYPES: &[&str] = &["EodTick", "OhlcTick", "QuoteTick", "TradeTick"];
...
fn render_python_tick_columnar(schema: &Schema) -> String {
    ...
    out.push_str("// Emitted only for the tick types called by the hand-written `*_df`\n");
    out.push_str("// wrappers in `lib.rs`; the generic `pyclass_list_to_columnar` covers\n");
    out.push_str("// the rest so we do not ship dead code.\n\n");
    for type_name in sorted_type_names(schema) {
        if !PYTHON_TICK_COLUMNAR_TYPES.contains(&type_name) {
            continue;
        }
        let def = &schema.types[type_name];
...
fn render_python_tick_classes(schema: &Schema) -> String {
    ...
    // Schema-derived column-name dispatcher.
    out.push_str(&render_python_tick_columns_for_qualname(schema));
    out
}

/// Emit `columns_for_qualname` — a static dispatcher that maps a pyclass
/// `__qualname__` (e.g. `"QuoteTick"`) to the tick's DataFrame column list
/// in schema-file order. Consumed by `pyclass_list_to_columnar` in `lib.rs`
/// so both empty and non-empty input lists yield a deterministic, schema-
/// pinned column set without runtime `__dir__` reflection.
fn render_python_tick_columns_for_qualname(schema: &Schema) -> String {
    let mut out = String::new();
    out.push_str("/// Static column-name schemas keyed by the tick pyclass `__qualname__`.\n");
    out.push_str("/// Used by `pyclass_list_to_columnar` to build DataFrame-ready column lists\n");
    out.push_str("/// in deterministic schema order, independent of Python runtime reflection.\n");
    out.push_str(
    ...
    for type_name in sorted_type_names(schema) {
        ...
        if def.contract_id {
            ...
            out.push_str("\"expiration\", \"strike\", \"right\"");
        }
        out.push_str("]),\n");
    }
    out.push_str("        _ => None,\n");
