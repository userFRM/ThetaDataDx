...
fn render_sdk_generated_files() -> Result<Vec<GeneratedSourceFile>, Box<dyn std::error::Error>> {
    ...
    Ok(vec![
        GeneratedSourceFile {
            relative_path: "sdks/python/src/tick_dicts.rs",
            contents: render_python_tick_dicts(&schema),
        },
        GeneratedSourceFile {
...
#[allow(dead_code)]
fn render_python_tick_dicts(schema: &Schema) -> String {
    ...
    for type_name in sorted_type_names(schema) {
        let def = &schema.types[type_name];
        out.push_str(&render_python_tick_dict_fn(type_name, def));
        out.push('\n');
    }
    out
}
...
#[allow(dead_code)]
fn render_python_tick_dict_fn(type_name: &str, def: &TickTypeDef) -> String {
    let mut out = String::new();
    let fn_name = match type_name {
        "EodTick" => "eod_tick_to_dict",
        "OhlcTick" => "ohlc_tick_to_dict",
        "TradeTick" => "trade_tick_to_dict",
        "QuoteTick" => "quote_tick_to_dict",
        "TradeQuoteTick" => "trade_quote_tick_to_dict",
        "OpenInterestTick" => "open_interest_tick_to_dict",
        "MarketValueTick" => "market_value_tick_to_dict",
        "GreeksTick" => "greeks_tick_to_dict",
        "IvTick" => "iv_tick_to_dict",
        "PriceTick" => "price_tick_to_dict",
        "CalendarDay" => "calendar_day_to_dict",
        "InterestRateTick" => "interest_rate_tick_to_dict",
        "OptionContract" => "option_contract_to_dict",
        other => panic!("unsupported Python tick dict type: {other}"),
    };

    // Historical endpoints now use columnar converters, but _to_dict functions
    // are retained for FPSS single-event returns and downstream consumers.
    out.push_str("#[allow(dead_code)]\n");
    writeln!(
        out,
        "fn {fn_name}(py: Python<'_>, value: &tick::{type_name}) -> Py<PyAny> {{"
    )
    .unwrap();
    out.push_str("    let dict = PyDict::new(py);\n");
    for column in &def.columns {
        let value_expr = if column.r#type == "String" {
            format!("&value.{}", column.field)
        } else {
            format!("value.{}", column.field)
        };
        writeln!(
            out,
            "    dict.set_item({:?}, {}).unwrap();",
            column.name, value_expr
        )
        .unwrap();
    }
    if type_name == "QuoteTick" {
        out.push_str("    dict.set_item(\"midpoint\", value.midpoint).unwrap();\n");
    }
    if def.contract_id {
        out.push_str("    set_contract_id!(dict, value);\n");
    }
...
fn go_public_field_name(type_name: &str, field: &str) -> &'static str {
    match (type_name, field) {
        ...
        (_, "iv_error") => "IVError",
        ("MarketValueTick", "shares_outstanding") => "SharesOut",
        ("MarketValueTick", "enterprise_value") => "EntValue",
        (_, "ms_of_day") => "MsOfDay",
        ...
        (_, "market_cap") => "MarketCap",
        (_, "shares_outstanding") => "SharesOutstanding",
        (_, "enterprise_value") => "EnterpriseValue",
        (_, "free_float") => "FreeFloat",
