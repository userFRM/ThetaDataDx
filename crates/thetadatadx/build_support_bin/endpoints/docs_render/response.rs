...
/// Render the sample-output table for an endpoint from its checked-in
/// capture fixture, when one exists. Returns `None` (schema only, no
/// fabricated data) when no capture is available.
pub(super) fn render_sample_section(
    endpoint: &GeneratedEndpoint,
    schema: &Schema,
) -> Result<Option<String>, Box<dyn std::error::Error>> {
    let Some(decoded) = super::samples::decode_capture(endpoint)? else {
        return Ok(None);
    };

    let def = tick_def(schema, &endpoint.return_type);
    let shown = decoded.rows.len();
    ...
    out.push_str(" |\n|");
    out.push_str(&"---|".repeat(def.columns.len()));
    out.push('\n');
    for row in &decoded.rows {
        assert_eq!(
            row.len(),
            def.columns.len(),
            "decoded row width does not match tick_schema for {}",
            endpoint.name
        );
        out.push_str("| ");
    ...
    out.push_str(&format!(
        "\nDecoded from a captured production response; {shown} of {} rows shown.\n\n",
        decoded.total_rows
    ));
    Ok(Some(out))
}
