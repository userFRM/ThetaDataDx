...
//!
//! Renders `sdks/python/src/_generated/historical_methods.rs` — the `ThetaDataDxClient` impl
...
//! cancelled. The `ThetaDataDxClient` handle remains usable for subsequent calls.
//!
...
pub(super) fn render_python_decode_bench(endpoints: &[GeneratedEndpoint]) -> String {
    ...
    out.push_str("/// the matching `ThetaDataDxClient` endpoint would have returned.\n");
    out.push_str("///\n");
...
pub(super) fn render_python_historical_methods(endpoints: &[GeneratedEndpoint]) -> String {
    ...
    // Per-endpoint fluent builder pyclasses. Emitted BEFORE the `ThetaDataDxClient`
...
fn render_python_endpoint_async(endpoint: &GeneratedEndpoint) -> String {
    ...
    // Clone the Arc<thetadatadx::ThetaDataDxClient> handle into the closure — the
...
// builder struct clones an `Arc<thetadatadx::ThetaDataDxClient>` from the owning
// `ThetaDataDxClient` pyclass (see lib.rs where the field is wrapped in Arc for
...
fn render_python_endpoint_builder(endpoint: &GeneratedEndpoint) -> String {
    ...
    // Same Arc the parent `ThetaDataDxClient` pyclass stores — cheap to clone,
    // so the async terminal can move it into the future.
    out.push_str("    tdx: std::sync::Arc<thetadatadx::ThetaDataDxClient>,\n");
    for param in &method_params {
        if param.param_type == "Symbols" {
