...
//! `TdxStreamEvent` wrapper embeds all of them by value; only the field
...
fn render_kind_enum_rust(schema: &Schema) -> String {
    ...
    out.push_str("/// `TdxStreamEvent` is valid. One discriminant per data variant\n");
    ...
    out.push_str("/// bump.\n");
    out.push_str("#[repr(C)]\n");
    out.push_str("pub enum TdxStreamEventKind {\n");
...
fn render_event_struct_rust(out: &mut String, event_name: &str, def: &EventDef) {
    ...
    out.push_str("#[repr(C)]\n");
    writeln!(out, "pub struct TdxStream{event_name} {{").unwrap();
...
fn render_event_zero_const(out: &mut String, event_name: &str, def: &EventDef) {
    ...
    writeln!(
        out,
        "pub(crate) const {const_name}: TdxStream{event_name} = TdxStream{event_name} {{"
    )
    .unwrap();
    if def.columns.is_empty() {
...
/// `TdxStreamEvent` for the Rust FFI crate.
///
...
pub(super) fn render_ffi_fpss_event_structs(schema: &Schema) -> String {
    ...
    out.push_str("// `TdxStreamEvent`. `include!`'d from `ffi/src/lib.rs`; do not hand-edit.\n");
    ...
    out.push_str("#[repr(C)]\n");
    out.push_str("pub struct TdxStreamEvent {\n");
    out.push_str("    /// Discriminant selecting which payload field below is valid.\n");
    out.push_str("    pub kind: TdxStreamEventKind,\n");
    for (event_name, _) in sorted_data_events(schema) {
        ...
        .unwrap();
        writeln!(out, "    pub {field}: TdxStream{event_name},").unwrap();
    }
...
/// `TdxStream{Variant}` field, captures the contract symbol into
...
fn render_data_arm(out: &mut String, schema: &Schema, event_name: &str, def: &EventDef) {
    ...
    out.push_str("                event: TdxStreamEvent {\n");
    writeln!(
        out,
        "                    kind: TdxStreamEventKind::{event_name},"
    )
    .unwrap();
    let field = snake_case(event_name);
    writeln!(out, "                    {field}: TdxStream{event_name} {{").unwrap();
    for column in &def.columns {
...
/// `TdxStream{Variant}` payload, captures any `String` into
...
/// field on the tagged `TdxStreamEvent` struct, skipping the active one.
...
fn render_unknown_control_helper(out: &mut String, schema: &Schema) {
    ...
    out.push_str("        FfiBufferedEvent {\n");
    out.push_str("            event: TdxStreamEvent {\n");
    out.push_str("                kind: TdxStreamEventKind::UnknownControl,\n");
