...
//! builds one directly from a borrowed `&fpss::FpssEvent`.
//!
...
pub(super) fn render_python_fpss_event_classes(schema: &Schema) -> String {
    ...
    out.push_str("// Typed #[pyclass] structs for every `FpssData` / `FpssControl`\n");
    out.push_str("// variant + `fpss_event_to_typed`, which builds one directly from a\n");
    out.push_str("// borrowed `&fpss::FpssEvent`. The single source of truth is\n");
    ...
    // Dispatcher: &FpssEvent -> typed pyclass for EVERY variant, in one
...
/// `&fpss::FpssEvent` to the typed pyclass. Mirrors the FFI converter
...
fn render_fpss_event_to_typed(schema: &Schema) -> String {
    ...
    out.push_str("    event: &fpss::FpssEvent,\n");
...
/// Emit one `FpssEvent::Data(FpssData::<Variant> { .. }) => ...` arm that
...
/// Emit one `FpssEvent::Control(FpssControl::<Variant> { .. }) => ...`
/// arm. The per-variant Rust pattern + field-construction mapping keeps
/// the translation from the core `FpssControl` enum's field shapes to the
...
fn render_control_arm(event_name: &str, def: &EventDef) -> String {
    ...
    if def.columns.is_empty() {
        writeln!(
            out,
            "            fpss::FpssControl::{rust_variant} => Py::new(py, {event_name}).map(|p| p.into_any()),",
        )
        .unwrap();
        return out;
    }
    ...
    writeln!(
        out,
        "            fpss::FpssControl::{rust_variant} {{ {rust_pattern} }} => Py::new(",
    )
    .unwrap();
    out.push_str("                py,\n");
    writeln!(out, "                {event_name} {{").unwrap();
