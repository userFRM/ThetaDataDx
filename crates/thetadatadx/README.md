...
historical (MDDS gRPC), real-time streaming (FPSS TCP), and flat-file
bulk pulls — without the JVM, the subprocess, or the local proxy.
One async client object speaks every transport directly.
...
- Direct h2 gRPC transport — no tonic, no tokio-tower layer.
- LMAX-Disruptor SPMC ring on the FPSS hot path; the TLS reader never
  blocks on user code.
