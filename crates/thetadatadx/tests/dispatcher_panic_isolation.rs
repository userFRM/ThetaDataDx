...
//! the Disruptor consumer via the test-only `FpssClient::for_self_join_test`
//! harness without a production FPSS TLS connection. See
//! `crates/thetadatadx/src/fpss/mod.rs` module `panic_isolation_tests`.
//!
//! This integration test verifies the public API shapes exposed on
//! `ThetaDataDxClient` and `FpssClient`. A successful build proves the
//! signatures exist; no live connection is required.

use thetadatadx::fpss::FpssClient;
use thetadatadx::ThetaDataDxClient;

/// Compile-time witness: `panic_count()` is reachable on `&ThetaDataDxClient`
...
/// Compile-time witness: `panic_count()` is reachable on `&FpssClient`
/// (the core streaming client) and returns `u64`.
#[test]
fn panic_count_accessor_present_on_fpss_client() {
    let _witness = |client: &FpssClient| {
