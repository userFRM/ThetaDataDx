...
fn direct_method_arg_name(endpoint: &GeneratedEndpoint, param: &GeneratedParam) -> String {
    ...
    match (endpoint.name.as_str(), param.name.as_str()) {
        ("stock_history_eod" | "option_history_eod" | "index_history_eod", "start_date") => {
            "start".into()
        }
        ("stock_history_eod" | "option_history_eod" | "index_history_eod", "end_date") => {
            "end".into()
        }
        _ => param.name.clone(),
    }
...
fn direct_optional_kind_and_default(param: &GeneratedParam) -> (&'static str, &'static str) {
    match param.name.as_str() {
        "start_time" => ("string", "\"09:30:00\".to_string()"),
        "end_time" => ("string", "\"16:00:00\".to_string()"),
        _ => match param.param_type.as_str() {
            "Int" => ("opt_i32", "None"),
            "Float" => ("opt_f64", "None"),
            "Bool" => ("opt_bool", "None"),
            _ => ("opt_str", "None"),
...
fn direct_list_column(endpoint: &GeneratedEndpoint) -> &'static str {
    if endpoint.name.ends_with("_list_symbols") {
        "symbol"
    } else if endpoint.name.ends_with("_list_dates") {
        "date"
    } else if endpoint.name.ends_with("_list_expirations") {
        "expiration"
    } else if endpoint.name.ends_with("_list_strikes") {
        "strike"
    } else {
        "value"
    }
