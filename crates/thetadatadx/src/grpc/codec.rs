    ...
    Req: Message,
{
    ...
    pub fn encode(req: &Req) -> Bytes {
        ...
        // reallocates. `encode` only fails if the buffer cannot hold the
        // message, which our `with_capacity` rules out — but we still
        // bubble the error rather than `.unwrap()` per workspace policy.
        match req.encode(&mut buf) {
            Ok(()) => buf.freeze(),
            // Pathological case: buffer ran out of room. We sized it to
            // `encoded_len`, so this fires only if prost lied about the
            // size — which would be a prost bug. Surface a fresh frame
            // with a zero-length payload rather than panic; the receiver
            // will fail to decode and surface a clean error.
            Err(_) => {
                let mut fallback = BytesMut::with_capacity(FRAME_HEADER_LEN);
                fallback.put_u8(0);
                fallback.put_u32(0);
                fallback.freeze()
            }
