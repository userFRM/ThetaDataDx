...
pub fn rows_to_arrow(rows: &[FlatFileRow]) -> Result<RecordBatch, Error> {
    if rows.is_empty() {
        ...
        return RecordBatch::try_new_with_options(schema, Vec::new(), &options)
            .map_err(|err| Error::Decode(format!("rows_to_arrow: {err}")));
    }
    ...
    for (row_idx, row) in rows.iter().enumerate() {
        ...
        if row.fields.len() != data_builders.len() {
            return Err(Error::Decode(format!(
                "rows_to_arrow: row {row_idx} has {} fields, expected {}",
                row.fields.len(),
                data_builders.len()
            )));
        }
            ...
            data_builders.iter_mut().zip(row.fields.iter()).enumerate()
        {
            if expected_name != actual_name {
                return Err(Error::Decode(format!(
                    "rows_to_arrow: row {row_idx} column {col_idx} named '{actual_name}', \
            ...
            match (builder, value) {
                ...
                (b, v) => {
                    return Err(Error::Decode(format!(
                        "rows_to_arrow: row {row_idx} column '{actual_name}' is {:?}, \
                         schema expects {:?}",
                        flatfile_value_arrow_type(v),
                        b.data_type()
                    )));
                }
            }
...
mod tests {
    ...
    fn schema_mismatch_field_count_errors() {
        ...
        assert!(msg.contains("1 fields"), "unexpected error: {msg}");
    }
    ...
    fn schema_mismatch_field_name_errors() {
        ...
        assert!(err.to_string().contains("named 'b'"));
    }
