...
/// - Stock/Index/Rate: ticker + security type
/// - Option: ticker + security type + expiration + call/put + strike
///
/// The public field names (`symbol`, `expiration`) follow the v3 vendor
/// surface; the binary wire format still calls the same bytes `root` /
/// `exp_date` in `Contract.toBytes()` / `Contract.fromBytes()` and that
/// internal byte-level naming is preserved verbatim in the doc comments
/// and parser locals to keep this file diff-able against the decompiled
/// terminal source. See:
...
pub struct Contract {
    /// Underlying ticker symbol (ASCII, max 16 chars on the wire).
    pub symbol: String,
    /// Security type.
...
impl Contract {
    ...
    fn parse_occ21(input: &str) -> Result<Self, Error> {
        ...
        // OCC-21 root field: bytes 0..6, right-padded with spaces. The
        // wire/OCC-21 layout calls this region the "root"; we keep the
        // local name matching the spec and assign it to the v3
        // `symbol` field on the Rust `Contract` struct below.
        let root_raw = &input[0..6];
        let symbol = root_raw.trim_end_matches(' ').to_string();
        Self::validate_root(input, &symbol)?;
    ...
    /// # Errors
    ///
    /// Returns [`Error::Config`] if the symbol is empty or longer than 16
    ...
    fn encode_unchecked(&self) -> Vec<u8> {
        // Wire format keeps the legacy "root" naming for the byte-level
        // length prefix and slice; the public Rust struct field is
        // `symbol` per the v3 vendor surface, so we read from
        // `self.symbol` and emit identical bytes.
    ...
    pub fn from_bytes(data: &[u8]) -> Result<(Self, usize), ContractParseError> {
        ...
        // Wire-format ASCII payload of the legacy "root" length-prefixed
        // slice. Bound to `symbol` here so the struct construction below
        // mirrors the v3 public field name.
        let symbol = std::str::from_utf8(&data[root_start..root_end])
        ...
        if sec_type == SecType::Option {
            ...
            let expiration = i32::from_be_bytes([
                data[opt_start],
                data[opt_start + 1],
                data[opt_start + 2],
                data[opt_start + 3],
...
impl std::str::FromStr for Contract {
    ...
    ///
    /// 1. **Bare symbol** (stock contract). 1..=16 ASCII uppercase letters,
    ...
    ///    The OCC-21 layout still calls the leading 6-byte field `root`;
    ///    on the Rust struct it lands in the `symbol` field. Both 21-char
    ///    (canonical) and 20-char (one space inside the root-padding
    ///    region) inputs are accepted. The 20-char form is repaired by
    ///    peeling off the fixed 15-char `YYMMDD+right+strike` suffix and
    ///    right-padding the leading root slice to 6 chars — byte-for-byte
    ///    equivalent to the 21-char form for the same contract, matching
    ///    the tolerant upstream feed that occasionally ships the 20-char
    ///    variant.
    ///    ```
    ///    # use std::str::FromStr;
    ///    # use thetadatadx::fpss::protocol::Contract;
    ...
    /// bad strike digits, empty symbol, non-uppercase character) and
