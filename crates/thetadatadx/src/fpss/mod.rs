...
/// is encoded as `Error::Streaming { kind: Disconnected }` with a
/// `"dispatcher failed: "` prefix on the message, which the reverse
/// direction recognises:
///
/// | `StreamError`               | `Error`                                                         |
/// |---------------------------|-----------------------------------------------------------------|
/// | `ConnectionRefused(m)`    | `Error::Streaming { kind: ConnectionRefused, message: m }`           |
/// | `Timeout(m)`              | `Error::Streaming { kind: Timeout, message: m }`                     |
/// | `Protocol(m)`             | `Error::Streaming { kind: ProtocolError, message: m }`               |
/// | `Disconnected(m)`         | `Error::Streaming { kind: Disconnected, message: m }`                |
/// | `RateLimited(m)`          | `Error::Streaming { kind: TooManyRequests, message: m }`             |
...
/// | `DispatcherFailed(m)`     | `Error::Streaming { kind: Disconnected, message: "dispatcher failed: {m}" }` |
///
...
/// FPSS-shaped umbrella variants (`Error::Streaming`, `Error::Auth`,
...
impl From<StreamError> for Error {
    fn from(e: StreamError) -> Self {
        ...
        match e {
            StreamError::ConnectionRefused(message) => Error::Streaming {
                kind: StreamErrorKind::ConnectionRefused,
                message,
            },
            StreamError::Timeout(message) => Error::Streaming {
                kind: StreamErrorKind::Timeout,
                message,
            },
            StreamError::Protocol(message) => Error::Streaming {
                kind: StreamErrorKind::ProtocolError,
                message,
            },
            StreamError::Disconnected(message) => Error::Streaming {
                kind: StreamErrorKind::Disconnected,
                message,
            },
            StreamError::RateLimited(message) => Error::Streaming {
            ...
            StreamError::DispatcherFailed(message) => Error::Streaming {
                kind: StreamErrorKind::Disconnected,
                message: format!("dispatcher failed: {message}"),
            },
            StreamError::ReentrantDrain(message) => Error::Streaming {
...
impl From<Error> for StreamError {
    fn from(e: Error) -> Self {
        ...
        match e {
            Error::Streaming { kind, message } => match kind {
...
impl StreamingClient {
    ...
    pub(in crate::fpss) fn send_cmd(&self, cmd: IoCommand) -> Result<(), Error> {
        self.cmd_tx
            ...
            .try_send(cmd)
            .map_err(|e| match e {
                std_mpsc::TrySendError::Full(_) => Error::Streaming {
                    kind: crate::error::StreamErrorKind::Disconnected,
                    message: format!(
                ...
                std_mpsc::TrySendError::Disconnected(_) => Error::Streaming {
                    kind: crate::error::StreamErrorKind::Disconnected,
