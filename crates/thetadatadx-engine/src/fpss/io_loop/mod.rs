    ...
    P: Producer<RingEvent>,
{
    ...
    for ctrl in pending_control.drain(..) {
            ...
            .is_err()
        {
            ...
            tracing::warn!(
                target: "thetadatadx_engine::fpss::io_loop",
...
/// `disruptor::builder::single::SPBuilder::new_event_poller`, so
/// **no** Disruptor consumer thread is spawned and **no** intermediate
...
/// and observes `disruptor::Polling::Shutdown` once it reaches that
/// sequence — the EOF-drain guarantee.
