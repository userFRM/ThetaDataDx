...
//!
//! # async fn doc() -> Result<(), thetadatadx_engine::Error> {
...
/// existing `thetadatadx_engine::endpoint::*` paths continue to resolve.
pub use mdds::endpoint_args as endpoint;

// `decode` is re-exported from `mdds::decode` to preserve the public surface
// (`thetadatadx_engine::decode::*`). The decode pipeline is split into
...
///
/// ```rust,no_run
/// use thetadatadx_engine::prelude::*;
/// # async fn doc() -> Result<(), thetadatadx_engine::Error> {
/// let creds  = Credentials::from_file("creds.txt")?;
...
/// #[global_allocator]
/// static GLOBAL: thetadatadx_engine::mimalloc::MiMalloc = thetadatadx_engine::mimalloc::MiMalloc;
/// ```
///
