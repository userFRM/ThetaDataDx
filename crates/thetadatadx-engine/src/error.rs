...
///
/// ```ignore
/// use thetadatadx_engine::error::{Error, ConfigErrorKind, DecodeErrorKind, GrpcStatusKind};
///
/// match err {
...
impl From<tdbe::error::Error> for Error {
    fn from(err: tdbe::error::Error) -> Self {
        ...
        // into the closest typed `thetadatadx_engine::Error` variant so callers
        // can use `?` when invoking `tdbe` APIs (e.g. `tdbe::right::parse_right`)
        // from a `Result<_, thetadatadx_engine::Error>` context. Every bridge
