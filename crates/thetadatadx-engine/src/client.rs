...
//!
//! ```rust,no_run
//! use thetadatadx_engine::{ThetaDataDxClient, Credentials, DirectConfig};
//!
//! #[tokio::main]
//! async fn main() -> Result<(), thetadatadx_engine::Error> {
...
/// Single-shot barrier the streaming dispatcher waits on before
/// entering its event loop. `start_streaming` opens the gate via
/// [`Self::go`] only after the streaming slot has been atomically
/// installed; the install-rejection rollback calls [`Self::abort`] so
/// the dispatcher exits without ever invoking the user callback.
struct StartupGate {
...
impl StartupGate {
    ...
    /// Block until [`Self::go`] or [`Self::abort`] is called. Returns
    /// `true` when the dispatcher should enter its iterator loop and
    ...
    fn wait_for_go(&self) -> bool {
        ...
        loop {
            ...
            guard = self.waker.wait(guard).unwrap_or_else(|e| e.into_inner());
        }
    }
}
...
pub struct ThetaDataDxClient {
    ...
    /// Single-flight gate held by [`Self::start_streaming`] and
    /// [`Self::reconnect_streaming`] for the duration of the
    /// connect-spawn-install sequence. Without this lock two concurrent
    /// starts could both pass the fast-path `is_streaming()` check,
    /// each build a fresh client and spawn its own dispatcher, then
    /// each overwrite [`Self::dispatcher_handle`] — the loser's
    /// dispatcher would be the one that eventually owned the `Live`
    /// slot but no `JoinHandle` would track it, breaking
    /// `stop_streaming` / `await_drain`.
    start_lock: Mutex<()>,
    /// Join handle for the background dispatcher thread that drains
    /// the `FpssClient` iterator and invokes the user callback. Set
    /// inside [`Self::start_streaming`] and joined inside
    /// [`Self::stop_streaming`] / on [`Drop`] so callers never observe
    /// a callback firing after quiescence.
    dispatcher_handle: Mutex<Option<std::thread::JoinHandle<()>>>,
    /// Set to `true` if the dispatcher thread terminated via a panic
    /// in the user callback rather than via the normal ring-shutdown
    /// drain. Read by [`Self::is_streaming`] /
    /// [`Self::connection_status`] so a panic surfaces as a visible
...
impl ThetaDataDxClient {
    ...
    pub async fn connect(creds: &Credentials, config: DirectConfig) -> Result<Self, Error> {
        ...
        Ok(Self {
            ...
            dispatcher_failed: Arc::new(AtomicBool::new(false)),
        })
    }
    ...
    {
        // Single-flight gate: serialise the entire connect-spawn-install
        // sequence so two concurrent starts cannot each spawn a
        // dispatcher and have the loser overwrite the winner's
        // `JoinHandle`. The lock is held across the FPSS connect call
        // (typically tens of milliseconds), but a second concurrent
        // start is rejected upfront either by the `is_streaming` fast
        // path below or by `install_live` once it observes a `Live`
        // slot — the lock just prevents the half-built session of one
        // from clobbering the other's bookkeeping.
        let _start_guard = self.start_lock.lock().unwrap_or_else(|e| e.into_inner());
        ...
        // Reset the dispatcher-failed flag for the fresh session.
        self.dispatcher_failed.store(false, Ordering::Release);
        ...
        let gate_for_dispatcher = Arc::clone(&startup_gate);
        let failure_flag = Arc::clone(&self.dispatcher_failed);
        let dispatcher_client = Arc::clone(&client_arc);
        let dispatcher_handle = std::thread::Builder::new()
            .name("tdx-fpss-dispatcher".into())
            .spawn(move || {
                if !gate_for_dispatcher.wait_for_go() {
                    return;
                }
                let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    let client_ref: &FpssClient = &dispatcher_client;
                    for event in client_ref {
                        match event {
                ...
                if outcome.is_err() {
                    ...
                    tracing::error!(
                        target: "thetadatadx_engine::client",
                        "tdx-fpss-dispatcher panicked; client transitioning to failed state",
                    );
                }
            })
        ...
        *self.dispatcher_handle.lock().unwrap_or_else(|e| {
            self.dispatcher_handle.clear_poison();
            e.into_inner()
        }) = Some(dispatcher_handle);

        // Run the install + gate signal under `catch_unwind` so a panic
        // (e.g. a poisoned `Mutex` chained off `dispatcher_handle`, an
        // `ArcSwap` allocator OOM, or any future-proofing hazard) cannot
        // leave the dispatcher waiting on `wait_for_go` forever and the
        // client wedged in `Drop`.
        ...
        let install_result = match install_attempt {
            Ok(r) => r,
            Err(panic_payload) => {
                // Signal the dispatcher to skip its iterator loop and
                // flag the failure flag so observers see a visible
                // failed state. Roll the freshly-built client back
                // before re-raising the panic.
                self.dispatcher_failed.store(true, Ordering::Release);
                startup_gate.abort();
                client_arc.shutdown();
                drop(client_arc);
                let pending = self
        ...
        match install_result {
            ...
            Err(install_err) => {
                ...
                // ring shutdown when it wakes up; signal the gate to
                // release with `abort` so the dispatcher skips its
                // iterator loop entirely.
                client_arc.shutdown();
                drop(client_arc);
                startup_gate.abort();
                let pending = self
    ...
    /// `thetadatadx_engine::fpss::io_loop`. Returns `0` when streaming has
    /// not started.
    #[must_use]
    ...
    pub fn is_streaming(&self) -> bool {
        match &**self.state.load() {
            StreamingSlot::Live { client: _ } => !self.dispatcher_failed.load(Ordering::Acquire),
    ...
    /// ```rust,no_run
    /// # use thetadatadx_engine::{ThetaDataDxClient, Credentials, DirectConfig};
    /// # use thetadatadx_engine::fpss::protocol::{Contract, SecTypeExt};
    /// # use tdbe::types::enums::SecType;
    /// # async fn doc(client: &ThetaDataDxClient) -> Result<(), thetadatadx_engine::Error> {
    ...
    pub fn stop_streaming(&self) {
        ...
        // `dispatcher_failed` is reset by `start_streaming` at the
        // start of every fresh session. Resetting it here would erase
        // a late panic from the just-stopped dispatcher before the
        // next session reads it, so leave the flag alone — observers
        // that read it while the slot is `Stopped` ignore it anyway
        // (see `is_streaming` / `connection_status`).
    }
    ...
    pub fn connection_status(&self) -> ConnectionStatus {
        match &**self.state.load() {
            ...
            StreamingSlot::Live { client } => {
                ...
                // with no deliveries".
                if self.dispatcher_failed.load(Ordering::Acquire) {
