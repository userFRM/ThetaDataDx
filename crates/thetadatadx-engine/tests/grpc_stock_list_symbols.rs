//! End-to-end test of [`thetadatadx_engine::grpc::stock_list_symbols`] against
...
use thetadatadx_engine::grpc::{stock_list_symbols, Channel};
use thetadatadx_engine::wire::{
...
async fn mid_stream_unauthenticated_classifies_as_grpc_unauthenticated() {
    ...
    // builder code path.
    use thetadatadx_engine::grpc::{ChannelError, Status};
    use thetadatadx_engine::wire::{DataValueList, ResponseData};
    ...
    let tdx_err: thetadatadx_engine::Error = ChannelError::Rpc {
        status: Status::new(16, "session expired"),
    }
    .into();
    match tdx_err {
        thetadatadx_engine::Error::Grpc {
            kind: thetadatadx_engine::error::GrpcStatusKind::Unauthenticated,
            ..
        } => {}
