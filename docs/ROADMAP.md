...
Empty cells (`—`) are intentional: the MCP / CLI / REST surfaces don't expose offline tick post-processing helpers because they are zero-cost in-process lookups already accessible to the client wrapping the binding.
...
- [x] **C** — `tdx_condition_name(...)`, `tdx_exchange_name(...)`, `tdx_sequence_signed_to_unsigned(...)`, etc. in `ffi/src/utility.rs`.
- [x] **C++** — `tdx::util::{condition_name, exchange_name, sequence_signed_to_unsigned, ...}` inline wrappers in `sdks/cpp/include/thetadx.hpp`.
