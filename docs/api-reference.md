...
**No JSON crosses the FFI boundary.** All inputs and outputs use typed `#[repr(C)]` structs -- historical endpoints, streaming events, Greeks, and subscriptions alike. Streaming events are delivered through `tdx_fpss_set_callback` / `tdx_unified_set_callback` (queued: callback runs on the LMAX Disruptor consumer thread under `catch_unwind`, ring-overflow drops counted on `tdx_*_dropped_events`) or `tdx_fpss_set_inline_callback` / `tdx_unified_set_inline_callback` (`expert-mode` opt-in, currently identical pipeline post-#513, microsecond-budget contract reserved for a future TLS-reader-direct dispatch). The user-supplied `extern "C" fn(const TdxFpssEvent*, void*)` receives a pointer to a tagged `#[repr(C)]` event struct (quote/trade/open_interest/ohlcvc/control/raw_data variants); the pointer is valid only for the duration of the callback.
...
| `tdx_fpss_set_callback` | `(handle, fn, ctx) -> i32` | Register queued callback (Disruptor consumer thread, `catch_unwind`-isolated) |
| `tdx_fpss_set_inline_callback` | `(handle, fn, ctx) -> i32` | Register inline callback (expert-mode, microsecond-budget contract) |
