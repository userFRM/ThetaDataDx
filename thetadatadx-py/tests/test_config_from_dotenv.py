...
def test_from_dotenv_api_key_only_keeps_production_host(tmp_path):
    ...
    assert cfg.market_data_host == "mdds-01.thetadata.us"


def test_from_dotenv_prod_and_stage_differ(tmp_path):
    prod_path = _write_dotenv(tmp_path, "prod.env", "THETADATA_MARKET_DATA_TYPE=PROD\n")
    stage_path = _write_dotenv(tmp_path, "stage2.env", "THETADATA_MARKET_DATA_TYPE=STAGE\n")
    prod = client.Config.from_dotenv(prod_path)
    stage = client.Config.from_dotenv(stage_path)
    assert prod.market_data_host != stage.market_data_host
