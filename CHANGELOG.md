...
- `parse_yyyymmdd` returns `Result<i32, Error>` with `Error::Config { kind: InvalidValue }` on parse failure; the previous implementation defaulted unparseable date strings to `0`.
