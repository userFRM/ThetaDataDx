...
The 13.0.0 line makes ThetaDataDx a terminal-exact, drop-in market-data SDK across Rust, Python, TypeScript, and C++, plus a bundled HTTP + WebSocket server and an MCP server, all over one Rust engine. It connects straight to ThetaData with nothing to install and run locally, and delivers US stock, option, index, and interest-rate data three ways from a single authenticated client: point-in-time history, real-time streaming, and whole-universe flat files. This release rebrands the entire public surface, unifies the client and error models across every binding, brings the bundled server to 1:1 parity with the ThetaData v3 terminal, and hardens streaming, decoding, and lifecycle handling throughout. It aggregates everything from the 13.0.0-rc.1 through 13.0.0-rc.18 pre-releases.

### Breaking changes

- **One brand across every surface.** The C++ header and library rename from `thetadx` to `thetadatadx` (`#include "thetadatadx.hpp"`, the C-ABI header `thetadatadx.h`, the `thetadatadx` CMake target). The C-ABI symbol prefix is `thetadatadx_` (was `tdx_`), C structs and handles carry the `ThetaDataDx` prefix (was `Tdx`), the C++ namespace is `thetadatadx` (was `tdx`), and the C preprocessor constants move from `TDX_` to `THETADATADX_` (values unchanged). No short-prefix aliases remain; recompilation and call-site renames are required.
- **Clients are named by role.** The unified client is `Client` (with Python's async companion `AsyncClient`); the single-purpose clients are `MarketDataClient` and `StreamingClient` on every binding. The prior long client name and the protocol-keyed `MddsClient` / `FpssClient` names are gone. The unified client exposes its market-data endpoints under a `market_data` sub-namespace and real-time streaming under a `stream` sub-namespace (`client.market_data.stock_history_eod(...)` — TypeScript `client.marketData`, Rust and C++ `client.market_data()` — and `client.stream.subscribe(...)`); flat files stay on the client directly. The streaming payload types are `StreamEvent` / `StreamData` / `StreamControl` (the `FpssEvent` family renamed in lockstep), the Rust streaming error is `StreamError` / `StreamErrorKind` (was `FpssError`), and the canonical Rust modules are `thetadatadx::streaming` and `thetadatadx::market_data` (the `fpss` / `mdds` paths remain as hidden compatibility aliases).
- **Config sections and environment variables are role-named.** Config sections are `config.market_data` and `config.streaming` (`MarketDataConfig` / `StreamingConfig`; were `mdds` / `fpss`). The two channels are selected independently: `MarketDataEnvironment` (`PROD` / `STAGE`) and `StreamingEnvironment` (`PROD` / `DEV`), with per-binding setters/getters and Python `market_data_type` / `streaming_type`, TypeScript `marketDataType` / `streamingType`. Environment variables follow: `THETADATA_MARKET_DATA_TYPE` / `THETADATA_STREAMING_TYPE` and `THETADATA_MARKET_DATA_HOST` / `_PORT`, `THETADATA_STREAMING_HOST` / `_PORT` (were `THETADATA_MDDS_*` / `THETADATA_FPSS_*`). An unrecognized or cross-channel value is now a hard error naming the key and the valid set. The server CLI renames `--mdds-region` / `--fpss-region` to `--market-data-region` / `--streaming-region`.
- **Field units and semantics carry one unambiguous name.** `strike` means dollars on every typed surface (`float` / `double`), with the exact wire integer reachable under the unit-named `strike_thousandths` (a `$550.00` strike is `550000`). The option `right` is the logical character (`'C'` / `'P'`, `uint32_t` Unicode scalar in C; both wire encodings decoded at the boundary). `EodTick` time columns are `created_ms_of_day` and `last_trade_ms_of_day` (were `ms_of_day` / `ms_of_day2`). `CalendarDay.is_open` is a boolean and `status` carries the `open` / `early_close` / `full_close` / `weekend` vocabulary (a `CalendarStatus` enum in Rust, `int32_t` + `thetadatadx_calendar_status_name()` in C). Absent contract identity is `None` / `undefined` (Arrow null) in Python and TypeScript rather than `0` / `""` sentinels; the C-layout rows keep their documented fills with `has_contract_id()`. `Contract.option(...)` takes the leg as one named `OptionLeg { expiration, strike, right }` form instead of three positional strings.
- **C-ABI struct layout.** `ThetaDataDxContract.strike` is `double` dollars at offset 24 with a trailing `int32_t strike_thousandths` at offset 32, growing the struct from 32 to 40 bytes; no existing field moves. Per-tick `right` fields are `uint32_t` Unicode scalars, and `ThetaDataDxCalendarDay.is_open` is C99 `bool`. Recompilation against the new header is required.
- **Typed, unified error taxonomy.** A `ConfigError` leaf (pinned to `THETADATADX_ERR_CONFIG`) carries environmental faults; `InvalidParameterError` is the leaf for rejected configuration, sequence, and flat-file inputs (new `THETADATADX_ERR_INVALID_PARAMETER = 13`); streaming faults route to `StreamError`. Python adds `NotFoundError` / `DeadlineExceededError` / `UnavailableError` (with `NoDataFoundError` / `TimeoutError` aliases), and `InvalidParameterError` inherits from both `ThetaDataError` and the built-in `ValueError`. Rate-limit errors expose the decoded server back-off (`RateLimitError.retry_after` / `retryAfter` / `retry_after()` and the C-ABI `thetadatadx_last_error_retry_after_ms()`).
- **TypeScript is fully asynchronous.** Every network entry point returns a `Promise` so a round-trip never blocks the Node event loop: the connect factories, all historical data-fetch methods, the streaming lifecycle (`startStreaming` / `reconnect`), and the flat-file methods. Each endpoint takes its required parameters positionally followed by one optional trailing options object (`stockHistoryEOD("AAPL", { startDate, endDate, timeoutMs })`), and the connect factories take a `Credentials` value plus an optional `Config`.
- **Surface restrictions and removals.** The flat-file surface is restricted to the datasets actually served — option `trade_quote` / `open_interest` / `eod` and stock `trade_quote` / `eod` — and an unsupported pair is rejected with a typed error before any network round-trip. The bundled `thetadatadx` CLI binary is removed. The client-side Black-Scholes Greeks calculator is removed across every binding; ThetaData's own served Greeks (the eod, trade, and snapshot greeks endpoints) are unchanged. The `concurrent_requests` knob is removed — historical concurrency is derived automatically from the account's subscription tier.
...
- **Intraday history serves a single date or a date range on the same route.** Matching the terminal, every intraday history endpoint (stock and option `ohlc` / `trade` / `quote` / `trade_quote`, option `open_interest`, the intraday greeks and trade-greeks families, and index `price`) takes an optional `date` plus optional `start_date` / `end_date`: supply `date` for a single day, or a range on the base route.
- **API-key authentication.** A key can be supplied inline, read from `THETADATA_API_KEY`, or loaded from a `.env` file, and it authenticates both market-data and streaming access across every SDK and the bundled server. New `Credentials` factories cover the sourcing options; email and password authentication is unchanged.
- **MCP server on npm.** The `thetadatadx-mcp` server runs with `npx -y thetadatadx-mcp` (no Rust toolchain required; `cargo install` stays for Rust users), and once authenticated it advertises only the tools the account's per-asset-class subscription grants.
- **Columnar and Arrow across every binding.** A pull-based Arrow `RecordBatch` reader, `batches()`, delivers decoded streaming events as Arrow record batches on a schedule you control (Rust `futures::Stream` + `.blocking()`, Python sync/async iterable over the Arrow C-Data interface, TypeScript `AsyncIterable<RecordBatch>`, C++ `arrow::RecordBatchReader`), tunable by `batch_size` and a `linger` flush with `Block` or `DropOldest` backpressure and a `dropped()` / `ring_dropped()` loss signal. Every history result gains a projected Arrow-IPC terminal that emits exactly the columns the wire carried, drivable from a live call via TypeScript's `<method>WithColumns` and the C / C++ column-presence out-param; multi-symbol snapshots attribute each row to its own `symbol`. Python DataFrame conversion releases the GIL and hands off zero-copy over the Arrow PyCapsule interface.
- **Live market value.** A per-contract theoretical bid / ask the SDK computes from the real-time quote, delivered as `StreamData::MarketValue` with `market_bid` / `market_ask` / integer-midpoint `market_price`, subscribed via `Contract::market_value()`.
- **Async query surfaces on every binding.** Python `AsyncClient` gains awaitable constructors and `*_async` flat-file twins; C++ gains an `<endpoint>_async(...)` companion returning `std::future` over the row type for every buffered query; and the C ABI and C++ gain market-data-result streaming through a tick-chunk callback so peak memory tracks one chunk.
- **Observability and delivery controls.** A slow-callback watchdog (`slow_callback_count()` + a microsecond threshold setter) counts over-budget callback invocations without ever cancelling one; an optional `consumer_cpu` knob pins the streaming consumer thread for low-jitter delivery; and epoch-instant accessors (`*_timestamp_ms`, plus `thetadatadx::time::date_ms_to_epoch_ms`) compute DST-aware Unix milliseconds on read from any row carrying a date and a milliseconds-of-day column.
- **`FlatFileFormat::Json` and `FlatFileFormat::Html`** join `Csv` and `Jsonl` on the flat-file SDK surface, a `from_file` client-construction convenience defaulting to the production configuration, sorted list-endpoint results, generated trade-flag accessors on every binding, and a machine-checked cross-binding parity contract (`parity.toml`) covering value-field types and the per-endpoint async and streaming families.

### Changed

- The workspace builds and publishes a single `thetadatadx` artifact: the `tdbe` time-and-calendar crate folds in as a private internal module, and the fixed-point price encoding (`Price` / `PriceType`) leaves the public surface entirely — every row already carries its price as decoded `f64` dollars.
- The default streaming read timeout is 10s (matching the terminal's socket read timeout) so a client rides through the brief silence after a full-market subscribe, and `StreamingClientBuilder` defaults its buffer to the production streaming size for full-universe overflow headroom.
- Flat-file and contract type fields render as stable wire tokens (`trade_quote`, `open_interest`, `eod`, `STOCK`, `OPTION`, `INDEX`, `RATE`) on every surface, decoupling the parsed token from internal identifiers, and columnar column names follow the public field names everywhere (`symbol`, `date`).

### Removed

- The `stock_history_ohlc_range` method and its `/v3/stock/history/ohlc_range` route (the terminal serves ranges on the base OHLC route), the bundled CLI, the client-side Greeks calculator, the `concurrent_requests` knob, the configurable ring wait-strategy tuning knobs, the `data_watchdog_ms` timer, the lingering Go SDK references, and the bundled server's own per-IP request rate limit (request limits are enforced upstream).

### Fixed

- **Streaming lifecycle and reconnect.** The reconnect budget resets only after a stable connected window (a flapping connection cannot reconnect forever), reconnect marks the session live only after replay succeeds and prefers the last-known-good host, an in-session server rate-limit or restart signal reaches the reconnect classifier, and delta-decode state resets on reconnect so the first post-reconnect ticks decode against a fresh baseline. Subscriptions are de-duplicated and reference-counted so a duplicate subscribe or a subscribe racing an in-flight reconnect under command-queue backpressure stays tracked and replayed exactly once. Read, write, and connect timeouts bound every socket operation, and a panicking I/O thread surfaces as a failure to callback, columnar, and pull consumers alike rather than a false "still streaming".
- **Teardown and memory safety across the bindings.** Teardown deadlocks are resolved on every binding: the C++ destructor routes through the drained teardown path (stopping a stream from inside its own callback can no longer read a destroyed callback, and async market-data views are lvalue-only so a call on a dangling temporary is a compile error), the Node.js client no longer deadlocks on close behind a slow callback or leaves a silently dead stream after `reconnect()`, and the Python client runs teardown off the GIL so a drop can no longer deadlock a re-entrant callback. A retired streaming callback is dropped only after the consumer thread confirms quiescence.
- **Decode and data integrity.** Truncated or drifted flat-file and EOD responses fail loud with a typed decode error instead of emitting garbage or zero-filled rows; a wrong-width streaming row is rejected before it poisons the per-contract field cache; market-value arithmetic saturates at the integer extremes instead of panicking on an adversarial quote; OHLCVC volume and count decode as unsigned so a large session stays positive; projected frames keep the trading `date`; and OCC-21 parsing and FIT integer runs no longer panic or silently drop digits on hostile input.
- **Configuration and connect.** Config invariants run at the single connect funnel every path routes through, the loader rejects a misspelled section, removed section, or unrecognized preset at load time (a valid section still falls back to production), a server retry hint is clamped to the configured ceiling, the C ABI installs its TLS crypto provider from every connect entrypoint, and the embedded async runtime honors the `worker_threads` knob rather than a hard-coded singleton.
- **Credentials and error hygiene.** Authentication errors carry only the HTTP status and never the upstream response body, the auth client no longer follows redirects, session UUIDs are redacted from `Debug` output, error cause-chains are preserved through `std::error::Error::source()`, and a null C-ABI callback pointer is rejected at registration instead of faulting on the first event.

### Security

- `pyo3` / `pyo3-async-runtimes` upgrade to 0.29, clearing RUSTSEC-2026-0176 and RUSTSEC-2026-0177 and letting the audit gate run with an empty ignore list; `memmap2` clears RUSTSEC-2026-0186; and the docs-site toolchain bumps `esbuild` for GHSA-gv7w-rqvm-qjhr. All are dependency and toolchain updates that never reached a shipped SDK API.
- Every client TLS configuration is built with an explicit `ring` crypto provider and explicit protocol versions rather than a process-global default, so `ring` is the sole provider in the graph and a connect no longer depends on an installed default.
...
- `MarketDataConfig.warn_on_buffered_threshold_bytes` (Rust `usize`, default 100 MiB) bound across every binding. A buffered historical response whose estimated size exceeds the threshold logs a single warning with `endpoint`, `row_count`, `bytes_est`, and `threshold_bytes` fields suggesting `.stream(handler)` for the workload. Fires once per request after the rows materialise; set to `0` to disable.
...
- Standalone `StreamingClient` and `MarketDataClient` pyclasses on the Python binding. `StreamingClient(creds, config)` opens only the streaming TLS transport (no historical gRPC channel, no Nexus HTTP auth); `MarketDataClient(creds, config)` opens only the historical gRPC channel plus Nexus authentication and surfaces the historical / FLATFILES API while raising `AttributeError` on every streaming-touching method. The bundled `Client` keeps its current behaviour; the new classes align the Python surface with the standalone clients already exposed by the C ABI (`thetadatadx_streaming_*` / `thetadatadx_client_*`) and the C++ wrapper (`thetadatadx::StreamingClient` / `thetadatadx::Client`).
...
- `MarketDataClient::open_channel_pool` routes `ChannelError -> Error` through the canonical `From<ChannelError> for Error` impl, dropping a hand-mapped duplicate `match` arm at the connect site. The channel-index context (`"channel {idx}: ..."`) is preserved on the `Transport`-shaped output.
...
- **Typed `SubscriptionTier` enum** (`Free`, `Value`, `Standard`, `Pro`)
  replacing raw `Option<i32>` on `MarketDataClient`.
...
- Mid-stream chunk header drift in the historical response accumulator was
  silently masked: `MarketDataClient::collect_stream` / `for_each_chunk` would
...
- 61 `MarketDataClient::<endpoint>_with_deadline` sibling methods on every
...
- `MarketDataClient::raw_query`, `MarketDataClient::raw_query_info`,
  `MarketDataClient::channel` — zero callers anywhere in the tree.
...
- **`DirectClient` renamed to `MarketDataClient`** (#383) — the historical-data gRPC client now carries the name of the service it actually speaks to (MDDS = Market Data Delivery Service). `use thetadatadx::DirectClient` call sites break; update to `use thetadatadx::MarketDataClient`. The `DirectConfig` associated config type keeps its name. High-level consumers of `Client` (Python / TypeScript / Go / C++ / Rust facade) are unaffected.
...
- **`DirectClient` renamed to `MarketDataClient`** — the struct inside the (now) `mdds/` module takes its module's name. Re-exported at the crate root as `thetadatadx::MarketDataClient`. `Client` still `Deref<Target = MarketDataClient>`s, so every historical endpoint method is reached unchanged via the unified client.
