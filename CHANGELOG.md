...
- **A stream stop racing an in-flight start no longer strands a live session (Python and TypeScript unified clients).** A `stop_streaming` / `close` that ran while `start_streaming` was still connecting cleared the registration but found nothing live to stop on the core, then the connect completed and left the core dispatcher firing the captured callback behind an empty binding slot, which also bypassed the double-start guard and delivered events past an observed close. The start now re-checks ownership after the connect and tears the freshly started session down if it was superseded, matching the standalone client.
