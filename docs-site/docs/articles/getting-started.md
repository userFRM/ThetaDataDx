...
- **Targeting staging?** Select the staging environment in any of three equivalent ways: the staging preset (`Config.stage()` / `DirectConfig::stage()`), the `THETADATA_MDDS_TYPE=STAGE` environment variable, or `DirectConfig::production().with_environment(Environment::Stage)` in Rust. Each one points authentication, historical, and streaming all at the staging cluster and works with either credential type. See [Configuration](/articles/configuration).
