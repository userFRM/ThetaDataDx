...
There are no handles. A buffer is a contract and a kind, opened by the first read of it and closed once fifteen minutes have passed without any of these tools using it, by the next call to one of them rather than on a timer, so nothing is released while the server sits idle. Using is wider than reading: a `live_prints` call keeps the trade and quote legs it reads from alive without moving either one's window. The first `live_read`, `live_prints` or `live_market` on a contract nothing was watching returns nothing yet and the second returns what arrived in between; a `live_prints` on a contract `live_read` already holds for its trades has them straight away, without the quotes beside them. `live_list` and `live_stop` answer immediately. Every answer reports the feed's own state alongside the rows, because a dead feed and a quiet contract look identical from an age alone. The rows carry the vendor's condition and exchange codes as they arrived, and the vendor's own bar is served as the vendor sent it, with the age of that bar beside it. Two things on this surface are computed rather than sent, and the tool returning each says so. `spread`, ask minus bid, which `live_market` can narrow and rank on and which is never a column on a row. And a `market_value` row in full: the vendor sends the underlying quote, and `market_bid`, `market_ask` and `market_price` are derived from it by the size-imbalance rule the vendor's own terminal uses, so they are a theoretical market value and not the quote. A quote or bar carried inside an answer has its own `age_ms` beside it, which is the age of that row and not of the answer.
