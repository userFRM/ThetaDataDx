...
- `thetadatadx::historical` is now the canonical module for the standalone historical client (`MarketDataClient`), mirroring `thetadatadx::streaming`. `MarketDataClient` and `SubscriptionTier` are now part of the default public Rust surface (previously feature-gated), so a Rust example builds a historical-only client from one place (`use thetadatadx::historical::MarketDataClient;`) and the type returned by `Client::historical()` is nameable. Both `thetadatadx::MarketDataClient` and `thetadatadx::historical::MarketDataClient` resolve. `thetadatadx::mdds` remains a hidden internal alias.
