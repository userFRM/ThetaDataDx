...
- **Clients are named by role.** The unified client is `Client` (with Python's async companion `AsyncClient`); the single-purpose clients are `HistoricalClient` and `StreamingClient` on every binding. The prior long client name and the protocol-keyed `MddsClient` / `FpssClient` names are gone. The unified client exposes historical data under a `historical` sub-namespace and real-time streaming under a `stream` sub-namespace (`client.historical.stock_history_eod(...)`, `client.stream.subscribe(...)`); flat files stay on the client directly. The streaming payload types are `StreamEvent` / `StreamData` / `StreamControl` (the `FpssEvent` family renamed in lockstep), the Rust streaming error is `StreamError` / `StreamErrorKind` (was `FpssError`), and the canonical Rust modules are `thetadatadx::streaming` and `thetadatadx::historical` (the `fpss` / `mdds` paths remain as hidden compatibility aliases).
- **Config sections and environment variables are role-named.** Config sections are `config.historical` and `config.streaming` (`HistoricalConfig` / `StreamingConfig`; were `mdds` / `fpss`). The two channels are selected independently: `HistoricalEnvironment` (`PROD` / `STAGE`) and `StreamingEnvironment` (`PROD` / `DEV`), with per-binding setters/getters and Python `historical_type` / `streaming_type`, TypeScript `historicalType` / `streamingType`. Environment variables follow: `THETADATA_HISTORICAL_TYPE` / `THETADATA_STREAMING_TYPE` and `THETADATA_HISTORICAL_HOST` / `_PORT`, `THETADATA_STREAMING_HOST` / `_PORT` (were `THETADATA_MDDS_*` / `THETADATA_FPSS_*`). An unrecognized or cross-channel value is now a hard error naming the key and the valid set. The server CLI renames `--mdds-region` / `--fpss-region` to `--historical-region` / `--streaming-region`.
...
- **API-key authentication.** A key can be supplied inline, read from `THETADATA_API_KEY`, or loaded from a `.env` file, and it authenticates both historical and streaming access across every SDK and the bundled server. New `Credentials` factories cover the sourcing options; email and password authentication is unchanged.
...
- **Async query surfaces on every binding.** Python `AsyncClient` gains awaitable constructors and `*_async` flat-file twins; C++ gains an `<endpoint>_async(...)` companion returning `std::future` over the row type for every buffered query; and the C ABI and C++ gain historical-result streaming through a tick-chunk callback so peak memory tracks one chunk.
