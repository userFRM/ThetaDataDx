...
- **A cross-channel environment selector surfaces as a typed error, not a panic.** Resolving the config from the environment with `THETADATA_MARKET_DATA_TYPE=DEV` (a value the historical channel does not support) now returns the `expected PROD or STAGE` error to the caller through the new `DirectConfig::try_production`, instead of unwinding inside the connect path.
...
- **Configuration:** the two server channels are selected independently and named after the data they serve. The historical channel (production or staging) and the streaming channel (production or dev) each have their own selector: `MarketDataEnvironment` / `StreamingEnvironment` in Rust, with `with_market_data_environment` / `with_streaming_environment` setters and `market_data_environment` / `streaming_environment` getters across every binding. The single combined environment selector is removed.
- **Environment variables:** `THETADATA_MDDS_TYPE` is renamed to `THETADATA_MARKET_DATA_TYPE` (`PROD` or `STAGE`) and `THETADATA_FPSS_TYPE` to `THETADATA_STREAMING_TYPE` (`PROD` or `DEV`). An unrecognized or cross-channel value (for example `THETADATA_MARKET_DATA_TYPE=DEV`) is now a hard error naming the offending key and the valid set rather than a silent fallback; a malformed host or port override is still skipped with a warning.
- **Server CLI:** `--mdds-region` is renamed to `--market-data-region` (`production` or `stage`) and `--fpss-region` to `--streaming-region` (`production` or `dev`). `--streaming-region` no longer accepts `stage`; there is no streaming staging cluster.
- **Bindings:** the inline-construction selectors are renamed to match — Python `market_data_type` / `streaming_type` and TypeScript `marketDataType` / `streamingType`.
...
- **Configuration:** host and environment selection is provenance-driven: an explicit host override (`THETADATA_MARKET_DATA_HOST`, `THETADATA_STREAMING_HOST` / `_PORT`, a config-file host list, or a `.env` entry) is tracked as a typed override and survives `stage()`, `dev()`, and `with_environment()`, with the most recent override winning and a config-file host list winning outright. `dev` is now a first-class environment with its own cluster and a production-equivalent auth marker, so an override on a dev config keeps the dev cluster. The `.env` reader covers every selection key with no split between clusters, and blank or quoted-whitespace values are ignored. A streaming configuration that resolves to no usable host is rejected rather than dialing an empty target.
...
- `thetadatadx::market_data` is now the canonical module for the standalone historical client (`MarketDataClient`), mirroring `thetadatadx::streaming`. `MarketDataClient` and `SubscriptionTier` are now part of the default public Rust surface (previously feature-gated), so a Rust example builds a historical-only client from one place (`use thetadatadx::market_data::MarketDataClient;`) and the type returned by `Client::market_data()` is nameable. Both `thetadatadx::MarketDataClient` and `thetadatadx::market_data::MarketDataClient` resolve. `thetadatadx::mdds` remains a hidden internal alias.
...
- Config sections are role-named to match the clients: `config.market_data` and `config.streaming` (Rust `MarketDataConfig` / `StreamingConfig`; the `DirectConfig` fields were `mdds` / `fpss`). The environment variables follow — `THETADATA_MARKET_DATA_HOST` / `_PORT` and `THETADATA_STREAMING_HOST` / `_PORT` (were `THETADATA_MDDS_*` / `THETADATA_FPSS_*`) — and the C-ABI config getters and setters rekey accordingly (`thetadatadx_config_get_market_data_host`, `thetadatadx_config_set_streaming_ring_size`, the full role-keyed family). The internal protocol modules keep their wire names; only the client-facing surface is role-named.
- Data surfaces — the unified client exposes historical data through a `market_data` sub-namespace and real-time streaming through a `stream` sub-namespace in every binding: Python getters (`client.market_data.stock_history_eod(...)`, `client.stream.subscribe(...)`), TypeScript getters (`client.marketData.stockHistoryEOD(...)`, `client.stream.startStreaming(...)`), and C++ view accessors (`client.market_data().stock_history_eod(...)`, `client.stream().set_callback(...)`). The Rust core routes through `client.market_data()` and `client.stream()`, and `StreamSurface` is exported from the crate root. Flat-file requests stay on the client directly (`client.flat_files` / `flat_files()`). The standalone `MarketDataClient` and `StreamingClient` keep their flat surfaces.
...
- TypeScript — every network entry point is asynchronous so a round-trip never blocks the Node event loop. The connect factories `Client.connect` / `Client.connectFromFile` / `MarketDataClient.connect` / `MarketDataClient.connectFromFile` return `Promise<Client>` / `Promise<MarketDataClient>`; the streaming lifecycle `startStreaming` / `reconnect` (on both the unified stream view and the standalone `StreamingClient`) return `Promise<void>`; and the flat-file methods (`optionTradeQuote` / `optionOpenInterest` / `optionEod` / `stockTradeQuote` / `stockEod` / `request` and `flatFileToPath`) return `Promise`. Call sites add `await`.
...
- `sdks/parity.toml` gains `[[value_field]]` rows — declared per-binding field TYPES on the load-bearing value classes, enforced by `scripts/check_binding_parity.py`, so a unit-bearing field type cannot silently drift between bindings again — and a `[[utility]]` section and `[[market_data_streaming]]` family so the offline calculators and the per-endpoint streaming terminals carry the same machine-checked cross-binding contract.
