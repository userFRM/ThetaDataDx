...
- **`parse_*_ticks` / `parse_option_contracts_v3` / `parse_calendar_days_v3` now return `Result<Vec<T>, DecodeError>`** (#325) -- the generated and hand-written row-decoders in `decode.rs` previously returned `Vec<T>` and silently coalesced per-cell type mismatches to zero. Type mismatches now propagate as `DecodeError::TypeMismatch { column, expected, observed }` which folds into `Error::Decode` at the `DirectClient` boundary. This is a Rust-surface breaking change for any caller that used these free functions directly; SDK users hit `DirectClient::*` methods whose `Result<Vec<T>, Error>` return shape is unchanged, so no ABI / FFI / Python / Go / C++ contract moves.
...
- **Unset `DataValue` oneof now fails loud in every strict decoder** -- `parse_option_contracts_v3` (expiration, right), `parse_calendar_days_v3` (date, type, open, close), and the generator-emitted EOD helpers plus contract-id injected `expiration` / `right` used to treat a `DataValue` whose `data_type` oneof was unset as a legitimate null and coalesce to `0`. They now return `DecodeError::TypeMismatch { observed: "Unset" }`, matching the behaviour of `row_number` / `row_date` / `row_float` / `row_text` / `row_number_i64` / `row_price_f64` and the Java terminal's `PojoMessageUtils.convert` default arm. `NullValue` is still coalesced (legitimate null); only the wire-anomaly path changes.
- **Bulletproof matrix foundation** (#290, PR #291) -- dropped the hand-maintained `endpoint_min_tier` and `endpoint_supports_expiration_wildcard` match statements in `build_support/endpoints.rs` in favor of generator-time lookups against a pinned upstream OpenAPI snapshot (`scripts/upstream_openapi.yaml`, 6758 lines, refreshable via `python3 scripts/check_tier_badges.py --refresh-snapshot`). The parser is a 30-line state machine in `build_support/upstream_openapi.rs` that fails closed on three classes of drift: (1) missing `x-min-subscription` on a real endpoint, (2) zero-endpoint snapshots, (3) zero references to either `expiration` or `expiration_no_star`, or any endpoint that references a param ref containing `expiration` that is not one of those two known variants. Surfaced one latent bug: `option_snapshot_market_value` was tagged `value` in the old map but upstream says `standard`; the regenerated validators now carry the correct declared tier. `scripts/check_tier_badges.py` now reads the committed snapshot by default with the live-fetch path preserved for `--refresh-snapshot` only.
- **Per-cell 60-second timeout on every validator** (#290, PR #291) -- every cell in all four live matrix validators is now bounded by a 60-second hard timeout. Stuck cells surface as `FAIL  timeout after 60s` instead of stalling CI indefinitely. Per-language safety semantics:
  - Python: raw daemon `threading.Thread` + `queue.Queue.get(timeout=60)` (NOT `concurrent.futures.ThreadPoolExecutor`, whose atexit joiner blocks interpreter shutdown on stuck non-daemon workers). Process exits via `os._exit()` to kill leaked daemon threads without running atexit handlers.
  - C++: `std::packaged_task` + detached `std::thread` + `future.wait_for(60s)`. If any cell timed out, `main` exits via `std::_Exit()` to skip destructors and avoid a use-after-free race between the leaked worker thread and `Client`/`Config`/`Credentials` RAII teardown.
  - Go: goroutine + `time.After(60s)` select + `errCellTimeout` sentinel. `ValidateAllEndpoints` now returns `(pass, skip, fail, hadTimeout, records)` so callers can skip `Close()` on the client handle when a timeout left a CGo call running against it; `sdks/go/cmd/validate/main.go` uses this signal.
  - CLI: `subprocess.run(..., timeout=60)` catching `TimeoutExpired`. Kernel kills the child; cleanest path.
  Fixture date range narrowed to a single day (`20250303`) to keep the bulk-chain × wildcard cells within the runtime budget. Widen when row-volume coverage matters more than runtime.
- **Validator tier gating and `expiration_no_star` coverage** -- reworked the generated matrix validators (`scripts/validate_python.py`, `validate_cli.py`, `sdks/go/validate.go`, `sdks/cpp/examples/validate.cpp`) so the live server drives SKIP classification instead of a client-side `VALIDATOR_ACCOUNT_TIER` env var that defaulted to `free`. Every cell is now attempted; `PermissionDenied` / `subscription` errors from the server are classified as `SKIP: tier-permission` with the declared min_tier echoed for context, and real bugs (invalid arguments, wire-format errors) continue to surface as `FAIL`. Also suppressed the wildcard-expiration modes (`all_exps_one_strike`, `bulk_chain`, `legacy_zero_wildcard`) on the 7 option endpoints that upstream binds to the `expiration_no_star` parameter (`option_snapshot_trade`, `option_history_ohlc`, and `option_history_greeks_{all,first_order,second_order,third_order,implied_volatility}`) -- the v3 server returns `Cannot specify '*' for the date` on those, so wildcard modes would always fail. Net cell count 185 (was 206, -21).
...
- **Per-optional-parameter mode coverage + cross-language agreement check** (#290, PR #291) -- the live matrix validators now emit a `with_<param>` mode for every optional (builder-bound) parameter each endpoint accepts, plus a compound `all_optionals` mode that sets every applicable optional at once. Compound pairs (`start_time`+`end_time` → `with_intraday_window`, `start_date`+`end_date` → `with_date_range` when both are optional on the endpoint) are single cells rather than two half-formed cells. Representative values follow the PR scope list (`max_dte=30`, `strike_range=10`, `venue="nqb"`, ...). Covered in Python / Go / C++; CLI intentionally skips per-optional modes because clap's positional-arg ordering makes targeted optional injection awkward (CLI still runs all other cells). No wire-shape deduplication — identical-on-the-wire cells stay distinct so the agreement check can pinpoint per-SDK drift. Cell count grew from 185 to 429 (Python, Go, C++ identical; CLI 185). New `scripts/validate_agreement.py` loads all four artifacts at `artifacts/validator_<lang>.json` (gitignored) and asserts every `(endpoint, mode)` cell present in ≥2 SDKs agrees on `status` and `row_count`; mismatches fail the release. `scripts/validate_release.sh` now runs CLI → Python → Go → C++ → agreement in that order. First-row hashing (sha256 of canonicalized JSON) is scaffolded in the record shape but not populated — deferred to a follow-up because it requires per-SDK tick introspection.
