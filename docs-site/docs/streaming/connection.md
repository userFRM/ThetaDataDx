...
Stocks and options are delivered as a single security-type-wide broadcast, so `SecType::Stock.full_trades()` / `SecType::Option.full_trades()` install one full-stream subscription. Indices have no such broadcast, so `SecType::Index.full_trades()` on the unified `ThetaDataDxClient` transparently expands to one per-contract subscription for every index root, enumerated once at connect — you get the complete index price tape with the same ergonomics, without naming roots by hand, and the expanded subscriptions ride the normal reconnect-replay path. Index price reports arrive on the trade channel and indices carry no open-interest stream, so `SecType::Index.full_open_interest()` is rejected with a configuration error. Rates have no full-stream broadcast and must be subscribed to per-contract (for example `Contract::rate("SOFR").trade()`); a rate full-stream subscription is rejected with a configuration error at `subscribe` time.
