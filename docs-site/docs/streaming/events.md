...
```
```python [Python]
# Push-callback delivery. The Disruptor consumer thread invokes
...
The unified `ThetaDataDxClient` dispatches every event through `start_streaming(callback)` (or the `streaming(callback)` context manager). The callable runs on the Disruptor consumer thread under the GIL, wrapped in `catch_unwind`.
...
| Method | Description |
|--------|-------------|
| `start_streaming(callback)` | Register a callable; the Disruptor consumer thread invokes `callback(event)` under the GIL for every typed FPSS event |
...
| `set_callback` | `(std::function<void(const FpssEvent&)>) -> void` | Register the push callback; the Disruptor consumer thread invokes it under `catch_unwind` |
