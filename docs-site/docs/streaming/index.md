...
Stocks and options are delivered as a single security-type-wide broadcast, so `SecType::Stock.full_trades()` and `SecType::Option.full_trades()` install one full-stream subscription. Indices have no such broadcast: `SecType::Index.full_trades()` on the unified `ThetaDataDxClient` transparently expands to one per-contract subscription per index root, enumerated at connect, so the index price tape arrives with the same ergonomics. Index price reports arrive on the trade channel and indices carry no open-interest stream, so `SecType::Index.full_open_interest()` is rejected with a configuration error. Rates have no full-stream broadcast either and must be subscribed to per-contract (for example `Contract::rate("SOFR").trade()`); a rate full-stream subscription is rejected with a configuration error when you call `subscribe`.
:::
