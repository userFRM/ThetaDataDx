...
```python
Client.historical.option_history_trade_quote(
...
```python
rows = client.historical.option_history_trade_quote(
    "SPY",
    "20250321",
    "20250303",
    strike="570",
...
```typescript
const rows = await client.historical.optionHistoryTradeQuote('SPY', '20250321', '20250303', { strike: '570', right: 'C' });
for (const t of rows) {
...
```cpp
auto rows = client.historical().option_history_trade_quote("SPY", "20250321", "20250303",
