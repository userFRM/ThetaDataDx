...
```python
rows = client.historical.option_history_greeks_all(
    "SPY",
    "20250321",
    "20250303",
    strike="570",
...
```typescript
const rows = await client.historical.optionHistoryGreeksAll('SPY', '20250321', '20250303', { strike: '570', right: 'C', interval: '1m' });
for (const t of rows) {
