...
pub fn option_history_greeks_first_order(
    ...
    expiration: &str,
    date: &str,
) -> OptionHistoryGreeksFirstOrderBuilder<'_>
```

Optional parameters chain on the builder: `.strike(&str)`, `.right(&str)`, `.interval(&str)`, `.start_time(&str)`, `.end_time(&str)`, `.annual_dividend(f64)`, `.rate_type(&str)`, `.rate_value(f64)`, `.version(&str)`, `.strike_range(i32)`, `.start_date(&str)`, `.end_date(&str)`. Execute with `.await` → `Result<Vec<GreeksFirstOrderTick>, Error>`, or decode chunk-by-chunk with `.stream(handler)`.
...
let rows = client
    .historical()
    .option_history_greeks_first_order("SPY", "20250321", "20250303")
    .strike("570")
    .right("C")
    .interval("1m")
    .await?;
...
```python
Client.historical.option_history_greeks_first_order(
...
`option_history_greeks_first_order_async(...)` awaits the same call shape. Chain `.to_pandas()` / `.to_polars()` / `.to_arrow()` / `.to_list()` on the result.

**Example**

```python
rows = client.historical.option_history_greeks_first_order(
    "SPY",
    "20250321",
    "20250303",
    strike="570",
...
optionHistoryGreeksFirstOrder(
  ...
  options?: { ... },
): Promise<Array<GreeksFirstOrderTick>>
```
...
```typescript
const rows = await client.historical.optionHistoryGreeksFirstOrder('SPY', '20250321', '20250303', { strike: '570', right: 'C', interval: '1m' });
for (const t of rows) {
...
```cpp
std::vector<GreeksFirstOrderTick> option_history_greeks_first_order(
...
```cpp
auto rows = client.historical().option_history_greeks_first_order("SPY", "20250321", "20250303",
...
```http
GET http://127.0.0.1:25503/v3/option/history/greeks/first_order
```
...
```bash
curl -G 'http://127.0.0.1:25503/v3/option/history/greeks/first_order' \
