...
```python
Client.historical.option_history_trade_greeks_first_order(
...
`option_history_trade_greeks_first_order_async(...)` awaits the same call shape. Chain `.to_pandas()` / `.to_polars()` / `.to_arrow()` / `.to_list()` on the result.

**Example**

```python
rows = client.historical.option_history_trade_greeks_first_order(
    "SPY",
    "20250321",
    "20250303",
    strike="570",
...
for t in rows:
    print(t.ms_of_day, t.price, t.delta, t.theta)
```
...
```typescript
optionHistoryTradeGreeksFirstOrder(
  symbol: string, expiration: string | Date, date: string | Date,
  options?: { ... },
): Promise<Array<TradeGreeksFirstOrderTick>>
```
...
```typescript
const rows = await client.historical.optionHistoryTradeGreeksFirstOrder('SPY', '20250321', '20250303', { strike: '570', right: 'C' });
for (const t of rows) {
  console.log(t.msOfDay, t.price, t.delta, t.theta);
}
```
...
```cpp
std::vector<TradeGreeksFirstOrderTick> option_history_trade_greeks_first_order(
...
```cpp
auto rows = client.historical().option_history_trade_greeks_first_order("SPY", "20250321", "20250303",
    thetadatadx::EndpointRequestOptions{}.with_strike("570").with_right("C"));
for (const auto& t : rows) {
    std::cout << t.ms_of_day << ' ' << t.price << ' ' << t.delta << ' ' << t.theta << "\n";
}
```
...
```http
GET http://127.0.0.1:25503/v3/option/history/trade_greeks/first_order
```
...
```bash
curl -G 'http://127.0.0.1:25503/v3/option/history/trade_greeks/first_order' \
