...
```python
ThetaDataDxClient.option_history_trade_greeks_first_order(
...
```python
rows = tdx.option_history_trade_greeks_first_order(
    "SPY",
    "20250321",
    "20250303",
    strike="570",
...
```typescript
const rows = await tdx.optionHistoryTradeGreeksFirstOrder('SPY', '20250321', '20250303', { strike: '570', right: 'C' });
for (const t of rows) {
...
```cpp
auto rows = client.option_history_trade_greeks_first_order("SPY", "20250321", "20250303",
