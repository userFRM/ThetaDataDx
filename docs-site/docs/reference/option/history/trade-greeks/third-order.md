...
pub fn option_history_trade_greeks_third_order(
    ...
    expiration: &str,
    date: &str,
) -> OptionHistoryTradeGreeksThirdOrderBuilder<'_>
```

Optional parameters chain on the builder: `.strike(&str)`, `.right(&str)`, `.start_time(&str)`, `.end_time(&str)`, `.annual_dividend(f64)`, `.rate_type(&str)`, `.rate_value(f64)`, `.version(&str)`, `.max_dte(i32)`, `.strike_range(i32)`, `.start_date(&str)`, `.end_date(&str)`. Execute with `.await` → `Result<Vec<TradeGreeksThirdOrderTick>, Error>`, or decode chunk-by-chunk with `.stream(handler)`.
...
let rows = client
    .historical()
    .option_history_trade_greeks_third_order("SPY", "20250321", "20250303")
    .strike("570")
    .right("C")
    .await?;
for t in &rows {
    println!("ms_of_day={} price={} speed={} zomma={}", t.ms_of_day, t.price, t.speed, t.zomma);
}
```
...
```python
Client.historical.option_history_trade_greeks_third_order(
...
`option_history_trade_greeks_third_order_async(...)` awaits the same call shape. Chain `.to_pandas()` / `.to_polars()` / `.to_arrow()` / `.to_list()` on the result.

**Example**

```python
rows = client.historical.option_history_trade_greeks_third_order(
    "SPY",
    "20250321",
    "20250303",
    strike="570",
...
optionHistoryTradeGreeksThirdOrder(
  ...
  options?: { ... },
): Promise<Array<TradeGreeksThirdOrderTick>>
```
...
```typescript
const rows = await client.historical.optionHistoryTradeGreeksThirdOrder('SPY', '20250321', '20250303', { strike: '570', right: 'C' });
for (const t of rows) {
...
```cpp
std::vector<TradeGreeksThirdOrderTick> option_history_trade_greeks_third_order(
...
```cpp
auto rows = client.historical().option_history_trade_greeks_third_order("SPY", "20250321", "20250303",
    thetadatadx::EndpointRequestOptions{}.with_strike("570").with_right("C"));
for (const auto& t : rows) {
    std::cout << t.ms_of_day << ' ' << t.price << ' ' << t.speed << ' ' << t.zomma << "\n";
}
```
...
```http
GET http://127.0.0.1:25503/v3/option/history/trade_greeks/third_order
```
...
```bash
curl -G 'http://127.0.0.1:25503/v3/option/history/trade_greeks/third_order' \
...
| `ms_of_day` | `sequence` | `ext_condition1` | `ext_condition2` | `ext_condition3` | `ext_condition4` | `condition` | `size` | `exchange` | `price` | `speed` | `zomma` | `color` | `ultima` | `implied_volatility` | `iv_error` | `underlying_ms_of_day` | `underlying_price` | `date` |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 34372940 | 1846852600 | 255 | 255 | 255 | 255 | 18 | 1 | 31 | 61.05 | 0 | -0.0145 | -0.0025 | -2394.8608 | 0.2001 | 0 | 34372000 | 565.13 | 20250502 |
