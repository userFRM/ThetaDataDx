...
```python
Client.historical.option_history_trade_greeks_second_order(
...
`option_history_trade_greeks_second_order_async(...)` awaits the same call shape. Chain `.to_pandas()` / `.to_polars()` / `.to_arrow()` / `.to_list()` on the result.

**Example**

```python
rows = client.historical.option_history_trade_greeks_second_order(
    "SPY",
    "20250321",
    "20250303",
    strike="570",
...
for t in rows:
    print(t.ms_of_day, t.price, t.gamma, t.vanna)
```
...
```typescript
optionHistoryTradeGreeksSecondOrder(
  symbol: string, expiration: string | Date, date: string | Date,
  options?: { ... },
): Promise<Array<TradeGreeksSecondOrderTick>>
```
...
```typescript
const rows = await client.historical.optionHistoryTradeGreeksSecondOrder('SPY', '20250321', '20250303', { strike: '570', right: 'C' });
for (const t of rows) {
  console.log(t.msOfDay, t.price, t.gamma, t.vanna);
}
```
...
```cpp
std::vector<TradeGreeksSecondOrderTick> option_history_trade_greeks_second_order(
...
```cpp
auto rows = client.historical().option_history_trade_greeks_second_order("SPY", "20250321", "20250303",
    thetadatadx::EndpointRequestOptions{}.with_strike("570").with_right("C"));
for (const auto& t : rows) {
    std::cout << t.ms_of_day << ' ' << t.price << ' ' << t.gamma << ' ' << t.vanna << "\n";
}
```
...
```http
GET http://127.0.0.1:25503/v3/option/history/trade_greeks/second_order
```
...
```bash
curl -G 'http://127.0.0.1:25503/v3/option/history/trade_greeks/second_order' \
...
| `ms_of_day` | `sequence` | `ext_condition1` | `ext_condition2` | `ext_condition3` | `ext_condition4` | `condition` | `size` | `exchange` | `price` | `gamma` | `vanna` | `charm` | `vomma` | `veta` | `implied_volatility` | `iv_error` | `underlying_ms_of_day` | `underlying_price` | `date` |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 34372940 | 1846852600 | 255 | 255 | 255 | 255 | 18 | 1 | 31 | 61.05 | 0.0036 | -0.6361 | 0.0004 | 162.7885 | 115.9427 | 0.2001 | 0 | 34372000 | 565.13 | 20250502 |
