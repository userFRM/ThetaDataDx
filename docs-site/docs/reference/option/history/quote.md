...
```python
ThetaDataDxClient.option_history_quote(
...
```typescript
const rows = await tdx.optionHistoryQuote('SPY', '20250321', '20250303', { strike: '570', right: 'C', interval: '1m' });
for (const t of rows) {
...
Optional parameters chain on `EndpointRequestOptions`: `.with_strike(...)`, `.with_right(...)`, `.with_interval(...)`, `.with_start_time(...)`, `.with_end_time(...)`, `.with_max_dte(...)`, `.with_strike_range(...)`, `.with_start_date(...)`, `.with_end_date(...)`. Throws `tdx::Error` on failure.

**Example**

```cpp
auto rows = client.option_history_quote("SPY", "20250321", "20250303",
