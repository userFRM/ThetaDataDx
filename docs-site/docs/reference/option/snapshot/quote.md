...
```python
ThetaDataDxClient.option_snapshot_quote(
    symbol, expiration,
    *,
...
```python
rows = tdx.option_snapshot_quote("SPY", "20250321", strike="570", right="C")
for t in rows:
...
```cpp
auto rows = client.option_snapshot_quote("SPY", "20250321",
