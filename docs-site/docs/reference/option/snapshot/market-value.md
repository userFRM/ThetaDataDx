...
```python
ThetaDataDxClient.option_snapshot_market_value(
    symbol, expiration,
    *,
...
```python
rows = tdx.option_snapshot_market_value("SPY", "20250321", strike="570", right="C")
for t in rows:
...
```typescript
const rows = await tdx.optionSnapshotMarketValue('SPY', '20250321', { strike: '570', right: 'C' });
for (const t of rows) {
...
```cpp
auto rows = client.option_snapshot_market_value("SPY", "20250321",
