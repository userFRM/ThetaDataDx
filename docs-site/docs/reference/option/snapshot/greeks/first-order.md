...
```rust
pub fn option_snapshot_greeks_first_order(&self, symbol: &str, expiration: &str) -> OptionSnapshotGreeksFirstOrderBuilder<'_>
```
...
`option_snapshot_greeks_first_order_async(...)` awaits the same call shape.

**Example**

```python
rows = client.historical.option_snapshot_greeks_first_order(
    "SPY",
    "20250321",
    strike="570",
    right="C",
...
```typescript
optionSnapshotGreeksFirstOrder(
...
```typescript
const rows = await client.historical.optionSnapshotGreeksFirstOrder('SPY', '20250321', { strike: '570', right: 'C' });
for (const t of rows) {
...
```cpp
std::vector<GreeksFirstOrderTick> option_snapshot_greeks_first_order(
...
```http
GET http://127.0.0.1:25503/v3/option/snapshot/greeks/first_order
```
...
```bash
curl -G 'http://127.0.0.1:25503/v3/option/snapshot/greeks/first_order' \
