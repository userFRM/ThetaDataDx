...
```python
rows = tdx.historical.option_snapshot_greeks_implied_volatility(
    "SPY",
    "20250321",
    strike="570",
    right="C",
...
```typescript
const rows = await tdx.historical.optionSnapshotGreeksImpliedVolatility('SPY', '20250321', { strike: '570', right: 'C' });
for (const t of rows) {
