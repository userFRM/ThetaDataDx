...
```python
Client.historical.option_snapshot_greeks_second_order(
    symbol, expiration,
    *,
...
```python
rows = client.historical.option_snapshot_greeks_second_order(
    "SPY",
    "20250321",
    strike="570",
    right="C",
...
```typescript
const rows = await client.historical.optionSnapshotGreeksSecondOrder('SPY', '20250321', { strike: '570', right: 'C' });
for (const t of rows) {
...
```cpp
auto rows = client.historical().option_snapshot_greeks_second_order("SPY", "20250321",
