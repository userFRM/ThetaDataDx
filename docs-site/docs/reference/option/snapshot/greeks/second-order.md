...
```python
rows = tdx.historical.option_snapshot_greeks_second_order(
    "SPY",
    "20250321",
    strike="570",
    right="C",
...
```typescript
const rows = await tdx.historical.optionSnapshotGreeksSecondOrder('SPY', '20250321', { strike: '570', right: 'C' });
for (const t of rows) {
