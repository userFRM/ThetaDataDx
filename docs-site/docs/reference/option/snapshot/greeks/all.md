...
```python
Client.historical.option_snapshot_greeks_all(
    symbol, expiration,
    *,
...
```python
rows = client.historical.option_snapshot_greeks_all(
    "SPY",
    "20250321",
    strike="570",
    right="C",
...
```typescript
const rows = await client.historical.optionSnapshotGreeksAll('SPY', '20250321', { strike: '570', right: 'C' });
for (const t of rows) {
...
```cpp
auto rows = client.historical().option_snapshot_greeks_all("SPY", "20250321",
