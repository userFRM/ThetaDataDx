...
```rust
let rows = client.historical().option_list_strikes("SPY", "20250321").await?;
for value in &rows {
    println!("{value}");
}
```
...
```python
rows = client.historical.option_list_strikes("SPY", "20250321")
for value in rows:
    print(value)
```
...
```typescript
const rows = await client.historical.optionListStrikes('SPY', '20250321');
