...
```rust
let rows = client.historical().stock_snapshot_trade(&["AAPL"]).await?;
for t in &rows {
...
```python
rows = client.historical.stock_snapshot_trade(["AAPL"])
for t in rows:
...
```typescript
const rows = await client.historical.stockSnapshotTrade(['AAPL']);
for (const t of rows) {
