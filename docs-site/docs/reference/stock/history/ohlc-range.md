...
```python
Client.historical.stock_history_ohlc_range(
...
```python
rows = client.historical.stock_history_ohlc_range(
    "AAPL",
    "20250303",
    "20250306",
    interval="1m",
...
```typescript
const rows = await client.historical.stockHistoryOHLCRange('AAPL', '20250303', '20250306', { interval: '1m' });
for (const t of rows) {
...
```cpp
auto rows = client.historical().stock_history_ohlc_range("AAPL", "20250303", "20250306",
