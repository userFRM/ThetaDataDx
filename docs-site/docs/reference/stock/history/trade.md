...
```rust
let rows = client.historical().stock_history_trade("AAPL", "20250303").await?;
for t in &rows {
...
```python
Client.historical.stock_history_trade(
    symbol, date,
    *,
...
```python
rows = client.historical.stock_history_trade("AAPL", "20250303")
for t in rows:
...
```typescript
const rows = await client.historical.stockHistoryTrade('AAPL', '20250303');
for (const t of rows) {
...
Optional parameters chain on `EndpointRequestOptions`: `.with_start_time(...)`, `.with_end_time(...)`, `.with_venue(...)`, `.with_start_date(...)`, `.with_end_date(...)`. Throws `thetadatadx::Error` on failure.

**Example**

```cpp
auto rows = client.historical().stock_history_trade("AAPL", "20250303");
