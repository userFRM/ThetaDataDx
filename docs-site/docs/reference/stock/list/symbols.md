...
```rust
let rows = client.historical().stock_list_symbols().await?;
for value in &rows {
    println!("{value}");
}
```
...
```python
Client.historical.stock_list_symbols(*, timeout_ms=None) -> StringList
```
...
```python
rows = client.historical.stock_list_symbols()
for value in rows:
    print(value)
```
...
```typescript
const rows = await client.historical.stockListSymbols();
...
```cpp
auto rows = client.historical().stock_list_symbols();
