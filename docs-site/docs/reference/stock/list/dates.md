...
```rust
let rows = client.historical().stock_list_dates("trade", "AAPL").await?;
for value in &rows {
    println!("{value}");
}
```
...
```python
Client.historical.stock_list_dates(
    request_type, symbol,
    *,
    timeout_ms=None,
) -> StringList
...
```python
rows = client.historical.stock_list_dates("trade", "AAPL")
for value in rows:
    print(value)
```
...
```typescript
const rows = await client.historical.stockListDates('trade', 'AAPL');
...
```cpp
auto rows = client.historical().stock_list_dates("trade", "AAPL");
