...
```rust
let rows = tdx.historical().stock_at_time_trade("AAPL", "20250303", "20250306", "10:30:00.000").await?;
for t in &rows {
...
```python
Client.historical.stock_at_time_trade(
...
```python
rows = tdx.historical.stock_at_time_trade("AAPL", "20250303", "20250306", "10:30:00.000")
for t in rows:
...
```typescript
const rows = await tdx.historical.stockAtTimeTrade('AAPL', '20250303', '20250306', '10:30:00.000');
for (const t of rows) {
...
```cpp
auto rows = client.historical().stock_at_time_trade("AAPL", "20250303", "20250306", "10:30:00.000");
