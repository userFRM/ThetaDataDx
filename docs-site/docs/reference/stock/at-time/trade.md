...
```rust
let rows = client.historical().stock_at_time_trade("AAPL", "20250303", "20250306", "10:30:00.000").await?;
for t in &rows {
...
```python
rows = client.historical.stock_at_time_trade(
    "AAPL",
    "20250303",
    "20250306",
    "10:30:00.000",
...
```typescript
const rows = await client.historical.stockAtTimeTrade('AAPL', '20250303', '20250306', '10:30:00.000');
for (const t of rows) {
