...
```rust
let rows = client.historical().index_list_symbols().await?;
for value in &rows {
    println!("{value}");
}
```
...
```python
Client.historical.index_list_symbols(*, timeout_ms=None) -> StringList
```
...
```python
rows = client.historical.index_list_symbols()
for value in rows:
    print(value)
```
...
```typescript
indexListSymbols(options?: { ... }): Promise<Array<string>>
```
...
```typescript
const rows = await client.historical.indexListSymbols();
...
```cpp
auto rows = client.historical().index_list_symbols();
