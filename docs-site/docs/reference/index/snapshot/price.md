...
```rust
let rows = client.historical().index_snapshot_price(&["SPX"]).await?;
for t in &rows {
...
```python
rows = client.historical.index_snapshot_price(["SPX"])
for t in rows:
...
```typescript
const rows = await client.historical.indexSnapshotPrice(['SPX']);
for (const t of rows) {
