...
The unified client registers a push callback for every typed `FpssEvent`. The Disruptor consumer thread invokes the callable for every event, wrapped in `catch_unwind` so a user-code panic is reported via the dropped-event counter instead of tearing down the consumer.
