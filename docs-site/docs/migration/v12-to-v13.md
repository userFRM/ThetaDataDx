...
The single combined environment selector is split into one selector per channel: `HistoricalEnvironment` (`Prod` / `Stage`) and `StreamingEnvironment` (`Prod` / `Dev`). The historical environment also drives the authentication marker; the streaming environment selects only the streaming hosts. There is no historical-dev cluster and no streaming-staging cluster, so an unrecognized or cross-channel value is a hard error that names the offending key and the valid set rather than falling back silently.
...
The selector environment variables follow the same split: `THETADATA_HISTORICAL_TYPE` (`PROD` / `STAGE`) and `THETADATA_STREAMING_TYPE` (`PROD` / `DEV`), which were `THETADATA_MDDS_TYPE` / `THETADATA_FPSS_TYPE`. The readback getters are `historical_environment` / `streaming_environment` (the combined `environment` getter is removed).
...
Each endpoint exports its `<Method>Options` interface from `index.d.ts`. The connect factories (`Client.connect` / `connectFromFile` and their `HistoricalClient` counterparts) and the streaming lifecycle methods `startStreaming` / `reconnect` resolve a `Promise`, so the network handshake and the streaming connect run off the event loop; `await` them. For memory-bounded pulls, the `<endpoint>Stream(..., callback)` companions deliver typed row chunks through a callback so peak memory tracks one chunk rather than the whole result.
