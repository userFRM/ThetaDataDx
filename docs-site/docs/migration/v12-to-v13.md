...
| TypeScript history methods | Now async, one trailing options object | `await client.marketData.stockHistoryEOD(sym, start, end, { ... })` |
...
| Config roles | `config.mdds` / `config.fpss` → `config.market_data` / `config.streaming` | Rename the sub-config reads (Rust `MarketDataConfig` / `StreamingConfig`) |
| Env vars | `THETADATA_MDDS_*` / `THETADATA_FPSS_*` → `THETADATA_MARKET_DATA_*` / `THETADATA_STREAMING_*` | Rename the exported variables |
| Environment selector | One combined selector → `MarketDataEnvironment` (`Prod`/`Stage`) + `StreamingEnvironment` (`Prod`/`Dev`); `THETADATA_MDDS_TYPE` / `FPSS_TYPE` → `THETADATA_MARKET_DATA_TYPE` / `STREAMING_TYPE` | Select per channel (`stage()` / `dev()` presets or `with_market_data_environment` / `with_streaming_environment`); invalid/cross-channel values now error |
| Server region flags | `--mdds-region` / `--fpss-region` → `--market-data-region` / `--streaming-region` | Update server invocations; `--streaming-region` drops `stage` |
...
const client = await Client.connect(new Credentials(email, password));
```
...
- **Python** — `Client` is the unified client; `AsyncClient` is its `await`-based companion (each historical endpoint is an `*_async` coroutine). `MarketDataClient` and `StreamingClient` are the role clients.
- **TypeScript** — `Client`, `MarketDataClient`, `StreamingClient`. See the connect section below for the `Credentials` value and the optional trailing `Config`.
- **C++** — `thetadatadx::Client`, `thetadatadx::MarketDataClient`, `thetadatadx::StreamingClient`.
- **Rust** — `thetadatadx::Client`, `thetadatadx::MarketDataClient`, `thetadatadx::StreamingClient`.

## Historical and streaming live under sub-namespaces

On the unified client, every historical / list / snapshot / at-time endpoint moves under a `market_data` sub-namespace, and the streaming lifecycle and subscription methods move under a `stream` sub-namespace. Flat-file requests stay on the client directly. The standalone `MarketDataClient` and `StreamingClient` keep their flat surfaces (no view indirection).
...
# v13
rows = client.market_data.stock_history_eod("AAPL", "20260601", "20260605")
...
// v13
const rows = await client.marketData.stockHistoryEOD("AAPL", "20260601", "20260605");
await client.stream.startStreaming(onEvent);
client.stream.subscribe(sub);
```
...
// v13
auto rows = client.market_data().stock_history_eod("AAPL", "20260601", "20260605");
...
// v13
let rows = client.market_data().stock_history_eod("AAPL", "20260601", "20260605").await?;
client.stream().subscribe(sub)?;
```

In Rust the unified client routes through `client.market_data()` and `client.stream()`, and the streaming view type `StreamSurface` is exported from the crate root. In Python `market_data` / `stream` and in TypeScript `marketData` / `stream` are property getters that return a fresh view over a cheap handle clone. In C++ `market_data()` / `stream()` return view objects.
...
The credentials-from-memory constructor is `thetadatadx_credentials_from_email` (the `(email, password)` order is unchanged). The role-specific connect entry points are `thetadatadx_market_data_connect` and `thetadatadx_streaming_connect`.

### C ABI: preprocessor constants

The public constants are renamed from the `TDX_` prefix to `THETADATADX_`; the integer values are unchanged, so only the names move. This covers `THETADATADX_ERR_*`, `THETADATADX_SUB_SCOPE_*` / `THETADATADX_SUB_KIND_*`, `THETADATADX_STREAM_*`, `THETADATADX_CALENDAR_STATUS_*`, `THETADATADX_RETRY_AFTER_NONE`, and `THETADATADX_ALIGN64_*`.
...
switch (event->kind) {
    case THETADATADX_STREAM_QUOTE:       handle_quote(event);  break;
    case THETADATADX_STREAM_PARSE_ERROR: handle_parse(event);  break;
}
```
...
// v13
let mut config = DirectConfig::production();
config.market_data.host = "10.0.0.5".into();
config.streaming.hosts = vec![("10.0.0.6".into(), 20000)];
```

The environment-variable overrides follow: `THETADATA_MARKET_DATA_HOST` / `THETADATA_MARKET_DATA_PORT` and `THETADATA_STREAMING_HOST` / `THETADATA_STREAMING_PORT` (were `THETADATA_MDDS_*` / `THETADATA_FPSS_*`).

```sh
# v12                                  # v13
export THETADATA_MDDS_HOST=10.0.0.5    export THETADATA_MARKET_DATA_HOST=10.0.0.5
export THETADATA_FPSS_HOST=10.0.0.6    export THETADATA_STREAMING_HOST=10.0.0.6
```

The C-ABI config getters and setters rekey accordingly — `thetadatadx_config_get_market_data_host`, `thetadatadx_config_set_streaming_ring_size`, and the rest of the role-keyed family.

## Environments are selected per channel

The single combined environment selector is split into one selector per channel: `MarketDataEnvironment` (`Prod` / `Stage`) and `StreamingEnvironment` (`Prod` / `Dev`). The market-data environment also drives the authentication marker; the streaming environment selects only the streaming hosts. There is no historical-dev cluster and no streaming-staging cluster, so an unrecognized or cross-channel value is a hard error that names the offending key and the valid set rather than falling back silently.

```rust
// v12 — one combined selector
let config = DirectConfig::production().with_environment(Environment::Stage);

// v13 — independent per channel (presets, or the explicit setters)
let config = DirectConfig::stage();                 // historical staging, streaming production
...
The selector environment variables follow the same split: `THETADATA_MARKET_DATA_TYPE` (`PROD` / `STAGE`) and `THETADATA_STREAMING_TYPE` (`PROD` / `DEV`), which were `THETADATA_MDDS_TYPE` / `THETADATA_FPSS_TYPE`. The readback getters are `market_data_environment` / `streaming_environment` (the combined `environment` getter is removed).

The bundled server CLI renames its region flags to match: `--market-data-region {production,stage}` and `--streaming-region {production,dev}` (were `--mdds-region` / `--fpss-region`); `--streaming-region` no longer accepts `stage`.
...
The umbrella Rust error variant `Error::Fpss { kind, message }` is renamed `Error::Stream { kind, message }` (its `Display` text is `stream error (...)`), and the C ABI streaming event-kind constants move from the `THETADATADX_FPSS_*` prefix to `THETADATADX_STREAM_*` (the `_TRADE` / `_QUOTE` / ... suffixes and the integer values are unchanged). The Python, TypeScript, and C++ `StreamError` class is unchanged.
...
quotes = client.market_data.option_history_quote("SPY", "20260620", "20260601")  # (symbol, expiration, date)
...
// v13
const rows = await client.marketData.stockHistoryTradeQuote('AAPL', '20260601', {
  exclusive: true,
  timeoutMs: 30000,
});
```

Each endpoint exports its `<Method>Options` interface from `index.d.ts`. The connect factories (`Client.connect` / `connectFromFile` and their `MarketDataClient` counterparts) and the streaming lifecycle methods `startStreaming` / `reconnect` resolve a `Promise`, so the network handshake and the streaming connect run off the event loop; `await` them. For memory-bounded pulls, the `<endpoint>Stream(..., callback)` companions deliver typed row chunks through a callback so peak memory tracks one chunk rather than the whole result.
...
const client = await Client.connect(creds, config);      // config is optional
const fromFile = await Client.connectFromFile("creds.txt", config);
```
...
In Python, `thetadatadx.Error` no longer exists: the event class is `ParseError` and the exception hierarchy roots at `ThetaDataError` (catch that, or a specific leaf like `NotFoundError`). In C the event kind is `THETADATADX_STREAM_PARSE_ERROR` and the payload struct is `ThetaDataDxStreamParseError`; the `ThetaDataDxStreamEventKind` discriminants renumber — recompile. C++ exposes `thetadatadx::StreamParseError`.
...
try:
    client.market_data.option_history_quote("SPY", "20260620", "20260601")
...
try {
  await client.marketData.optionHistoryQuote("SPY", "20260620", "20260601");
} catch (e) {
...
The `tdbe` time-and-calendar crate is folded into `thetadatadx` as a private internal module, so the workspace builds and publishes a single `thetadatadx` artifact. The curated public surface is unchanged in shape — `TradeTick`, `SecType`, `utils::conditions`, and the calendar types are reached at `thetadatadx::*` paths — but any downstream crate that imported from `tdbe::` directly must move to the `thetadatadx::*` path:

```rust
// v12
use tdbe::types::enums::SecType;
use tdbe::utils::conditions;

// v13
use thetadatadx::SecType;
use thetadatadx::utils::conditions;
```

The client-side Black-Scholes Greeks calculator that v12 exposed as `greeks::all_greeks` is removed: Greeks are served by ThetaData over gRPC and land on the EOD, trade, and snapshot tick rows (`GreeksEodTick`, `TradeGreeksAllTick`, and the per-order tick types), so the SDK no longer recomputes them client-side.
...
## New: streaming consumer-core pinning

This is an addition, not a breaking change: the default reproduces the previous behaviour exactly, so existing code needs no edits. The event-ring consumer runs a fixed low-latency wait (spin, yield, then a `spin_loop` hint — never sleeps), and one opt-in knob lets you pin its thread to a CPU core.

The optional `consumer_cpu` knob pins the tick-consumer thread to a CPU core for deterministic, low-jitter delivery; leave it unset (the default) to keep the thread under the OS scheduler.
...
This generic override is Rust-only by design: `wait_for` fires on every ring-empty poll on the hot path, so routing a per-poll callback across the C ABI, the CPython lock, or the JavaScript event loop would be a latency regression. The bindings run the fixed low-latency wait.
