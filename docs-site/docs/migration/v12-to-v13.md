...
- **C** — `is_open` is C99 `bool`; `status` stays `int32_t` with the `TDX_CALENDAR_STATUS_*` constants and the `thetadatadx_calendar_status_name()` lookup.
