...
- **C** — `TdxContract.strike` is `double` dollars. Offset and struct size are unchanged (the field widens into former tail padding); recompile against the new header. The C++ `tdx::strike_dollars(c)` helper is now `tdx::strike(c)`.
