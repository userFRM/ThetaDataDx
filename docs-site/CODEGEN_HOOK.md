...
| Per-endpoint pages (everything under `docs/historical/`) and the one-shot `api-reference.md` | the SDK endpoint surface spec | Hand-authored today; **must migrate to codegen** |
...
The script reads the SDK endpoint surface spec, emits one markdown partial per endpoint to `docs-site/docs/historical/<kind>/<name>/_params.md`, and pages include it via VitePress's `<!--@include: ./_params.md-->`.
