...
All three tools read email/password credentials from a `creds.txt` file (`--creds <path>`, default `creds.txt`); the MCP server also reads the `THETA_EMAIL` / `THETA_PASSWORD` environment variables. The REST / WebSocket server additionally accepts a ThetaData API key via the `--api-key` flag or the `THETADATA_API_KEY` environment variable. MCP and server accept an offline mode when credentials are absent.
