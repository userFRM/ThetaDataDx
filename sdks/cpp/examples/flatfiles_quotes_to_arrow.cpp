...
int main() {
    try {
        ...
        std::cout << "option_quote rows: " << rows.size() << std::endl;
        ...
        std::ofstream out("/tmp/option-quote.arrow", std::ios::binary);
        ...
        std::cout << "wrote " << ipc.size() << " bytes of Arrow IPC to "
                  << "/tmp/option-quote.arrow" << std::endl;
        ...
        std::cout << "raw vendor CSV at /tmp/option-quote.csv" << std::endl;
