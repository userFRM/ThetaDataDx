...
func (c *Client) StockHistoryTradeStream(symbol string, date string, opts ...EndpointOption) ([]TradeTick, error) {
	cSymbol := C.CString(symbol)
	defer C.free(unsafe.Pointer(cSymbol))
	cDate := C.CString(date)
	defer C.free(unsafe.Pointer(cDate))
	cOpts, freeOpts := endpointRequestOptionsToC(collectEndpointRequestOptions(opts))
	defer freeOpts()
	arr := C.tdx_stock_history_trade_stream_with_options(c.handle, cSymbol, cDate, cOpts)
	result := convertTradeTicks(arr)
	C.tdx_trade_tick_array_free(arr)
	return result, nil
}

func (c *Client) StockHistoryQuoteStream(symbol string, date string, interval string, opts ...EndpointOption) ([]QuoteTick, error) {
	cSymbol := C.CString(symbol)
	defer C.free(unsafe.Pointer(cSymbol))
	cDate := C.CString(date)
	defer C.free(unsafe.Pointer(cDate))
	...
	cOpts, freeOpts := endpointRequestOptionsToC(collectEndpointRequestOptions(opts))
	defer freeOpts()
	arr := C.tdx_stock_history_quote_stream_with_options(c.handle, cSymbol, cDate, cInterval, cOpts)
	result := convertQuoteTicks(arr)
	C.tdx_quote_tick_array_free(arr)
	return result, nil
}

func (c *Client) OptionHistoryTradeStream(symbol string, expiration string, strike string, right string, date string, opts ...EndpointOption) ([]TradeTick, error) {
	cSymbol := C.CString(symbol)
	defer C.free(unsafe.Pointer(cSymbol))
	cExpiration := C.CString(expiration)
	defer C.free(unsafe.Pointer(cExpiration))
	...
	cOpts, freeOpts := endpointRequestOptionsToC(collectEndpointRequestOptions(opts))
	defer freeOpts()
	arr := C.tdx_option_history_trade_stream_with_options(c.handle, cSymbol, cExpiration, cStrike, cRight, cDate, cOpts)
	result := convertTradeTicks(arr)
	C.tdx_trade_tick_array_free(arr)
	return result, nil
}

func (c *Client) OptionHistoryQuoteStream(symbol string, expiration string, strike string, right string, date string, interval string, opts ...EndpointOption) ([]QuoteTick, error) {
	cSymbol := C.CString(symbol)
	defer C.free(unsafe.Pointer(cSymbol))
	cExpiration := C.CString(expiration)
	defer C.free(unsafe.Pointer(cExpiration))
	...
	cOpts, freeOpts := endpointRequestOptionsToC(collectEndpointRequestOptions(opts))
	defer freeOpts()
	arr := C.tdx_option_history_quote_stream_with_options(c.handle, cSymbol, cExpiration, cStrike, cRight, cDate, cInterval, cOpts)
	result := convertQuoteTicks(arr)
	C.tdx_quote_tick_array_free(arr)
	return result, nil
}
