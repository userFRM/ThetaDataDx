...
impl ThetaDataDx {
    ...
    fn stock_history_trade_stream(
        ...
        let ticks = py.detach(|| {
            let mut request = self.tdx.stock_history_trade_stream(symbol, date);
            if let Some(value) = start_time {
                request = request.start_time(value);
            }
            if let Some(value) = end_time {
    ...
    fn stock_history_quote_stream(
        ...
        let ticks = py.detach(|| {
            let mut request = self.tdx.stock_history_quote_stream(symbol, date, interval);
            if let Some(value) = start_time {
                request = request.start_time(value);
            }
            if let Some(value) = end_time {
    ...
    fn option_history_trade_stream(
        ...
        let ticks = py.detach(|| {
            let mut request = self.tdx.option_history_trade_stream(symbol, expiration, strike, right, date);
            if let Some(value) = start_time {
                request = request.start_time(value);
            }
            if let Some(value) = end_time {
    ...
    fn option_history_quote_stream(
        ...
        let ticks = py.detach(|| {
            let mut request = self.tdx.option_history_quote_stream(symbol, expiration, strike, right, date, interval);
            if let Some(value) = start_time {
                request = request.start_time(value);
            }
            if let Some(value) = end_time {
