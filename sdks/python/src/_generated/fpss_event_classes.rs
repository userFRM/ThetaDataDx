...
// Typed #[pyclass] structs for every `FpssData` / `FpssControl`
// variant + `fpss_event_to_typed`, which builds one directly from a
// borrowed `&fpss::FpssEvent`. The single source of truth is
...
pub(crate) fn fpss_event_to_typed(
    ...
    match event {
        ...
        fpss::FpssEvent::Control(ctrl) => match ctrl {
            fpss::FpssControl::Connected => Py::new(py, Connected).map(|p| p.into_any()),
            fpss::FpssControl::ContractAssigned { id, contract } => Py::new(
                py,
                ContractAssigned {
                    id: *id,
                    contract: (**contract).clone(),
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::Disconnected { reason } => Py::new(
                py,
                Disconnected {
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::LoginSuccess { permissions } => Py::new(
                py,
                LoginSuccess {
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::MarketClose => Py::new(py, MarketClose).map(|p| p.into_any()),
            fpss::FpssControl::MarketOpen => Py::new(py, MarketOpen).map(|p| p.into_any()),
            fpss::FpssControl::Error { message } => Py::new(
                py,
                ParseError {
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::Ping { payload } => Py::new(
                py,
                Ping {
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::Reconnected => Py::new(py, Reconnected).map(|p| p.into_any()),
            fpss::FpssControl::ReconnectedServer => Py::new(py, ReconnectedServer).map(|p| p.into_any()),
            fpss::FpssControl::Reconnecting { reason, attempt, delay_ms } => Py::new(
                py,
                Reconnecting {
                    reason: *reason as i32,
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::ReconnectsExhausted { reason, attempts } => Py::new(
                py,
                ReconnectsExhausted {
                    reason: *reason as i32,
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::ReqResponse { req_id, result } => Py::new(
                py,
                ReqResponse {
                    req_id: *req_id,
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::Restart => Py::new(py, Restart).map(|p| p.into_any()),
            fpss::FpssControl::ServerError { message } => Py::new(
                py,
                ServerError {
                    message: message.clone(),
                },
            ...
            .map(|p| p.into_any()),
            fpss::FpssControl::UnknownFrame { code, payload } => Py::new(
                py,
                UnknownFrame {
                    code: *code,
                    payload: payload.clone(),
