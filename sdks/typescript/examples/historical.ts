...
async function main(): Promise<void> {
  ...
  const eod = await client.stockHistoryEOD("AAPL", "20240101", "20240301");
  ...
  const bars = await client.stockHistoryOHLC("AAPL", "20240315", {
    interval: "60000",
  });
  ...
  const exps = await client.optionListExpirations("SPY");
  console.log(`  Next 5: ${exps.slice(0, 5).join(", ")}\n`);

  // Option strikes.
  if (exps.length > 0) {
    const strikes = await client.optionListStrikes("SPY", exps[0]);
